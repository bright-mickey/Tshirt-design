function DashboardController($scope,$rootScope,$modal,$http,$location,$state,DashboardService,AnimationService,_,$popover,$sce,Upload,$timeout,$interval){function collision($div1,$div2){var x1=$div1.offset().left,y1=$div1.offset().top,h1=$div1.outerHeight(!0),w1=$div1.outerWidth(!0),b1=y1+h1,r1=x1+w1,x2=$div2.offset().left,y2=$div2.offset().top,h2=$div2.outerHeight(!0),w2=$div2.outerWidth(!0),b2=y2+h2,r2=x2+w2;return!(b1-h1<y2||y1>b2-h1||r1-w1<x2||x1>r2-w1)}function launch_compaign(){if(vm.front_design.length||vm.back_design.length||vm.right_design.length||vm.left_design.length){for(var to_be_sent=[],shirts=_.where(vm.shirts_in_styles,{selected:!0}),i=0;i<shirts.length;i++){to_be_sent[i]={},to_be_sent[i].id=shirts[i].id,to_be_sent[i].price=shirts[i].total_unit_cost+shirts[i].profit;for(var colors=_.where(shirts[i].colors_available,{selected:!0}),clr_ids=[],j=0;j<colors.length;j++)clr_ids.push(colors[j].color_id);to_be_sent[i].colors=clr_ids}var defaultProduct={};defaultProduct.id=vm.selectedShirt.id;for(var clrs_default=[],i=0;i<vm.selectedShirt.colors_available.length;i++){var color=vm.selectedShirt.colors_available[i];color.selected&&clrs_default.push(color.color_id)}for(var tags_arr="",i=0;i<vm.tags.length-1;i++)tags_arr=tags_arr+vm.tags[i]+",";tags_arr+=vm.tags[vm.tags.length-1],defaultProduct.colors=clrs_default;var compaign_json={};compaign_json.name=vm.compaign_title,compaign_json.url=vm.url,compaign_json.description=vm.desc,compaign_json.length=vm.compaign_length,compaign_json.products=JSON.stringify(to_be_sent),compaign_json.defaultProduct=JSON.stringify(defaultProduct),compaign_json.tags=JSON.stringify(tags_arr),compaign_json.category_id=vm.description.category,compaign_json.tipping_point=vm.goal_target,console.log("jskldfjklasdf",compaign_json.description);for(var sides=[],activeSide=1===vm.default_tab?"front":2===vm.default_tab?"back":3===vm.default_tab?"left":"right",counter=0,i=0;i<4;i++){if(0==i){var printableAreaWidth=$(".printable-area").css("width"),printableAreaHeight=(printableAreaWidth.substr(0,printableAreaWidth-2),$(".printable-area").css("height"));printableAreaHeight.substr(0,printableAreaHeight-2);console.log(printableAreaWidth,printableAreaHeight);var svgParent=document.createElementNS("http://www.w3.org/2000/svg","svg");svgParent.setAttribute("style","border: 1px solid black"),svgParent.setAttribute("width","220"),svgParent.setAttribute("height","330"),svgParent.setAttributeNS("http://www.w3.org/2000/xmlns/","xmlns:xlink","http://www.w3.org/1999/xlink"),svgParent.setAttributeNS("http://www.w3.org/2000/xmlns/","xmlns","http://www.w3.org/2000/svg"),svgParent.setAttributeNS(null,"viewBox","0 0 220 330");var foriegnObject=document.createElementNS("http://www.w3.org/2000/svg","foreignObject");foriegnObject.setAttribute("width","220"),foriegnObject.setAttribute("height","330");var mainDiv=document.createElement("div");mainDiv.setAttributeNS("http://www.w3.org/2000/xmlns/","xmlns","http://www.w3.org/1999/xhtml");var printDiv=document.createElement("div");printDiv.setAttribute("style","width: 220px;height:330px;overflow:hidden"),mainDiv.appendChild(printDiv);for(var layers=[],j=0;j<vm.front_design.length;j++){var art=$(".front").clone();art.find(".zoom-view,.visual-text,ul").remove(),art.css("position","relative").removeAttr("class").removeAttr("ng-repeat").removeAttr("id"),art.find(".user-box").css("position","absolute").removeAttr("on-drag-end").removeAttr("ng-style").removeAttr("id").removeAttr("class").removeAttr("ng-class").removeAttr("on-drag"),art.find("svg").removeAttr("ng-show").removeAttr("ng-if").removeAttr("ng-width").removeAttr("ng-height"),art.find("image").removeAttr("ng-click").removeAttr("ng-width").removeAttr("ng-height"),"img"!==vm.front_design[j].type&&art.find(".svgimg").remove(),printDiv.appendChild(art[j])}foriegnObject.appendChild(mainDiv),svgParent.appendChild(foriegnObject),console.log(svgParent);for(var k=0;k<vm.front_design.length;k++){if("text"===vm.front_design[k].type){var obj_=vm.front_design[k];console.log(vm.front_design[k].z_index);var svgText,element=$("#box"+vm.front_design[k].z_index+"front"),svg=element.children("svg")[0],view=svg.getAttributeNS(null,"viewBox"),arr=view.split(" ");svgText=$("#box"+vm.front_design[k].z_index+"front text").clone()[0],svgText.children[0].setAttributeNS(null,"x",parseInt(element.css("left"))),svgText.children[0].setAttributeNS(null,"y",parseInt(element.css("top"))),svgText.children[0].setAttributeNS(null,"width",parseInt(element.css("width"))),svgText.children[0].setAttributeNS(null,"height",parseInt(element.css("height")));var textObj={string:vm.front_design[k].text,font:{id:k,family:vm.front_design[k].font,filename:vm.front_design[k].font,tags:[],priority:1,url:null},fontSize:24,tag:"Popular",lineHeight:1.2,alignment:"center",colors:obj_.color,outlineThickness:0,transformations:[],matrix:{},beingEdited:!1,origin:"Native",type:"Text",bbox:{x:parseInt(arr[0]),y:parseInt(arr[1]),width:parseInt(arr[2]),height:parseInt(arr[3])},filename:"","preserve-colors":!1,fillColor:{id:k,value:vm.front_design[k].color,origin:null,heather:!1,texture:""},outlineColor:{id:"default"+k,value:"#000000",origin:null,heather:!1,texture:""},newDesignData:{transform:{}}};layers.push(textObj)}if("svg"===vm.front_design[k].type||"img"===vm.front_design[k].type){counter++;var svgText,element=$("#box"+vm.front_design[k].z_index+"front");svgText=element.find("image").clone()[0],svgText.setAttributeNS(null,"x",parseInt(element.css("left"))),svgText.setAttributeNS(null,"y",parseInt(element.css("top"))),svgText.setAttributeNS(null,"width",parseInt($("#box"+vm.front_design[k].z_index+"front svg").css("width"))),svgText.setAttributeNS(null,"height",parseInt($("#box"+vm.front_design[k].z_index+"front svg").css("height")));var obj_=vm.front_design[k],_obj=_.findWhere(vm.graphics,{id:obj_.id}),element=$("#box"+vm.front_design[k].z_index+"front"),svg=element.find("svg"),view=svg[0].getAttributeNS(null,"viewBox"),arr=view.split(" "),svgObj={urlParams:{filename:""},name:"",format:"",original_key:"",sourceFilename:"",svg_key:"",png_key:"",category:[],uploadId:null,initialScale:1,transformations:[],matrix:{},beingEdited:!1,origin:"Native",type:"Art",bbox:{x:parseInt(arr[0]),y:parseInt(arr[1]),width:parseInt(arr[2]),height:parseInt(arr[3])},filename:"","preserve-colors":!1,fillColor:{id:k,value:vm.front_design[k].color,origin:null,heather:!1,texture:""},outlineColor:{id:"default"+k,value:"#000000",origin:null,heather:!1,texture:""},newDesignData:{transform:{}}};"svg"===vm.front_design[k].type&&(svgObj.colors=_obj.colors),layers.push(svgObj)}}console.log("Front"),svgParent=svgParent.outerHTML,svgParent=svgParent.replace(/\r?\n|\r/g," "),svgParent=svgParent.replace(/<!--(.*?)-->/gi,""),svgParent=svgParent.replace(/\t/g,""),console.log(svgParent);var front={removeLayerBinding:{},name:"front",layers:layers,colors:vm.total_front_colors,bbox:{x:parseInt(arr[0]),y:parseInt(arr[1]),width:parseInt(arr[2]),height:parseInt(arr[3])},absoluteBBox:{x:parseInt(arr[0]),y:parseInt(arr[1]),width:parseInt(arr[2]),height:parseInt(arr[3])},beingViewed:!1,initialFreeTransformAttrs:null,initialPrintableArea:null,referencePoint:{},ppi:18,editable:null,sequence:2,scaleAndMove:null,rasterImageInfo:[],svg:svgParent};sides.push(front)}if(1==i){var svgParent1=document.createElementNS("http://www.w3.org/2000/svg","svg");svgParent1.setAttribute("style","border: 1px solid black"),svgParent1.setAttribute("width","220"),svgParent1.setAttribute("height","330"),svgParent1.setAttributeNS("http://www.w3.org/2000/xmlns/","xmlns:xlink","http://www.w3.org/1999/xlink"),svgParent1.setAttributeNS("http://www.w3.org/2000/xmlns/","xmlns","http://www.w3.org/2000/svg"),svgParent1.setAttributeNS(null,"viewBox","0 0 220 330");var foriegnObject=document.createElementNS("http://www.w3.org/2000/svg","foreignObject");foriegnObject.setAttribute("width","220"),foriegnObject.setAttribute("height","330");var mainDiv=document.createElement("div");mainDiv.setAttributeNS("http://www.w3.org/2000/xmlns/","xmlns","http://www.w3.org/1999/xhtml");var printDiv=document.createElement("div");printDiv.setAttribute("style","width: 220px;height:330px;overflow:hidden"),mainDiv.appendChild(printDiv);for(var layers=[],j=0;j<vm.back_design.length;j++){var art=$(".back").clone();art.find(".zoom-view,.visual-text,ul").remove(),art.css("position","relative").removeAttr("class").removeAttr("ng-repeat").removeAttr("id"),art.find(".user-box").css("position","absolute").removeAttr("on-drag-end").removeAttr("ng-style").removeAttr("id").removeAttr("class").removeAttr("ng-class").removeAttr("on-drag"),art.find("svg").removeAttr("ng-show").removeAttr("ng-if").removeAttr("ng-width").removeAttr("ng-height"),art.find("image").removeAttr("ng-click").removeAttr("ng-width").removeAttr("ng-height"),"img"!==vm.back_design[j].type&&art.find(".svgimg").remove(),printDiv.appendChild(art[j])}foriegnObject.appendChild(mainDiv),svgParent1.appendChild(foriegnObject),console.log(svgParent1);for(var k=0;k<vm.back_design.length;k++){if("text"===vm.back_design[k].type){var svgText,obj_=vm.back_design[k],element=$("#box"+vm.front_design[k].z_index+"back"),svg=element.children("svg")[0],view=svg.getAttributeNS(null,"viewBox"),arr=view.split(" ");svgText=$("#box"+vm.back_design[k].z_index+"back text").clone()[0],console.log(svgText),svgText.children[0].setAttributeNS(null,"x",parseInt(element.css("left"))),svgText.children[0].setAttributeNS(null,"y",parseInt(element.css("top"))),svgText.children[0].setAttributeNS(null,"width",parseInt(element.css("width"))),svgText.children[0].setAttributeNS(null,"height",parseInt(element.css("height")));var textObj={string:vm.back_design[k].text,font:{id:k,family:vm.back_design[k].font,filename:vm.back_design[k].font,tags:[],priority:1,url:null},fontSize:24,tag:"Popular",lineHeight:1.2,alignment:"center",colors:obj_.color,outlineThickness:0,transformations:[],matrix:{},beingEdited:!1,origin:"Native",type:"Text",bbox:{x:parseInt(arr[0]),y:parseInt(arr[1]),width:parseInt(arr[2]),height:parseInt(arr[3])},filename:"","preserve-colors":!1,fillColor:{id:k,value:vm.back_design[k].color,origin:null,heather:!1,texture:""},outlineColor:{id:"default"+k,value:"#000000",origin:null,heather:!1,texture:""},newDesignData:{transform:{}}};layers.push(textObj)}if("svg"===vm.back_design[k].type||"img"===vm.back_design[k].type){var svgText,element=$("#box"+vm.back_design[k].z_index+"back");svgText=element.find("image").clone()[0],svgText.setAttributeNS(null,"x",parseInt(element.css("left"))),svgText.setAttributeNS(null,"y",parseInt(element.css("top"))),svgText.setAttributeNS(null,"width",parseInt($("#box"+vm.back_design[k].z_index+"back svg").css("width"))),svgText.setAttributeNS(null,"height",parseInt($("#box"+vm.back_design[k].z_index+"back svg").css("height")));var obj_=vm.back_design[k],_obj=_.findWhere(vm.graphics,{id:obj_.id}),element=$("#box"+vm.back_design[k].z_index+"back"),svg=element.find("svg"),view=svg[0].getAttributeNS(null,"viewBox"),arr=view.split(" "),svgObj={urlParams:{filename:""},name:"",format:"",original_key:"",sourceFilename:"",svg_key:"",png_key:"",category:[],uploadId:null,initialScale:1,transformations:[],matrix:{},beingEdited:!1,origin:"Native",type:"Art",bbox:{x:parseInt(arr[0]),y:parseInt(arr[1]),width:parseInt(arr[2]),height:parseInt(arr[3])},filename:"","preserve-colors":!1,fillColor:{id:k,value:vm.back_design[k].color,origin:null,heather:!1,texture:""},outlineColor:{id:"default"+k,value:"#000000",origin:null,heather:!1,texture:""},newDesignData:{transform:{}}};"svg"===vm.back_design[k].type&&(svgObj.colors=_obj.colors),layers.push(svgObj)}}console.log("Back"),svgParent1=svgParent1.outerHTML,svgParent1=svgParent1.replace(/\r?\n|\r/g," "),svgParent1=svgParent1.replace(/<!--(.*?)-->/gi,""),svgParent1=svgParent1.replace(/\t/g,""),console.log(svgParent1);var back={removeLayerBinding:{},name:"back",layers:layers,colors:vm.total_back_colors,bbox:{x:parseInt(arr[0]),y:parseInt(arr[1]),width:parseInt(arr[2]),height:parseInt(arr[3])},absoluteBBox:{x:parseInt(arr[0]),y:parseInt(arr[1]),width:parseInt(arr[2]),height:parseInt(arr[3])},beingViewed:!1,initialFreeTransformAttrs:null,initialPrintableArea:null,referencePoint:{},ppi:18,editable:null,sequence:2,scaleAndMove:null,rasterImageInfo:[],svg:svgParent1};sides.push(back)}if(2==i){var svgParent2=document.createElementNS("http://www.w3.org/2000/svg","svg");svgParent2.setAttribute("style","border: 1px solid black"),svgParent2.setAttribute("width","189"),svgParent2.setAttribute("height","313"),svgParent2.setAttributeNS("http://www.w3.org/2000/xmlns/","xmlns:xlink","http://www.w3.org/1999/xlink"),svgParent2.setAttributeNS("http://www.w3.org/2000/xmlns/","xmlns","http://www.w3.org/2000/svg"),svgParent2.setAttributeNS(null,"viewBox","0 0 189 313");var foriegnObject=document.createElementNS("http://www.w3.org/2000/svg","foreignObject");foriegnObject.setAttribute("width","189"),foriegnObject.setAttribute("height","313");var mainDiv=document.createElement("div");mainDiv.setAttributeNS("http://www.w3.org/2000/xmlns/","xmlns","http://www.w3.org/1999/xhtml");var printDiv=document.createElement("div");printDiv.setAttribute("style","width: 189px;height:313px;overflow:hidden"),mainDiv.appendChild(printDiv);for(var layers=[],j=0;j<vm.left_design.length;j++){var art=$(".left").clone();art.find(".zoom-view,.visual-text,ul").remove(),art.css("position","relative").removeAttr("class").removeAttr("ng-repeat").removeAttr("id"),art.find(".user-box").css("position","absolute").removeAttr("on-drag-end").removeAttr("ng-style").removeAttr("id").removeAttr("class").removeAttr("ng-class").removeAttr("on-drag"),art.find("svg").removeAttr("ng-show").removeAttr("ng-if").removeAttr("ng-width").removeAttr("ng-height"),art.find("image").removeAttr("ng-click").removeAttr("ng-width").removeAttr("ng-height"),"img"!==vm.left_design[j].type&&art.find(".svgimg").remove(),printDiv.appendChild(art[j])}foriegnObject.appendChild(mainDiv),svgParent2.appendChild(foriegnObject),console.log(svgParent2);for(var k=0;k<vm.left_design.length;k++){if("text"===vm.left_design[k].type){var svgText,obj_=vm.left_design[k],element=$("#box"+vm.left_design[k].z_index+"left"),svg=element.children("svg")[0],view=svg.getAttributeNS(null,"viewBox"),arr=view.split(" ");svgText=$("#box"+vm.left_design[k].z_index+"left text").clone()[0],console.log(svgText),svgText.children[0].setAttributeNS(null,"x",parseInt(element.css("left"))),svgText.children[0].setAttributeNS(null,"y",parseInt(element.css("top"))),svgText.children[0].setAttributeNS(null,"width",parseInt(element.css("width"))),svgText.children[0].setAttributeNS(null,"height",parseInt(element.css("height")));var textObj={string:vm.left_design[k].text,font:{id:k,family:vm.left_design[k].font,filename:vm.left_design[k].font,tags:[],priority:1,url:null},fontSize:24,tag:"Popular",lineHeight:1.2,alignment:"center",colors:obj_.color,outlineThickness:0,transformations:[],matrix:{},beingEdited:!1,origin:"Native",type:"Text",bbox:{x:parseInt(arr[0]),y:parseInt(arr[1]),width:parseInt(arr[2]),height:parseInt(arr[3])},filename:"","preserve-colors":!1,fillColor:{id:k,value:vm.left_design[k].color,origin:null,heather:!1,texture:""},outlineColor:{id:"default"+k,value:"#000000",origin:null,heather:!1,texture:""},newDesignData:{transform:{}}};layers.push(textObj)}if("svg"===vm.left_design[k].type||"img"===vm.left_design[k].type){var svgText,element=$("#box"+vm.left_design[k].z_index+"left");svgText=element.find("image").clone()[0],svgText.setAttributeNS(null,"x",parseInt(element.css("left"))),svgText.setAttributeNS(null,"y",parseInt(element.css("top"))),svgText.setAttributeNS(null,"width",parseInt($("#box"+vm.left_design[k].z_index+"left svg").css("width"))),svgText.setAttributeNS(null,"height",parseInt($("#box"+vm.left_design[k].z_index+"left svg").css("height")));var obj_=vm.left_design[k],_obj=_.findWhere(vm.graphics,{id:obj_.id}),element=$("#box"+vm.left_design[k].z_index+"left"),svg=element.find("svg"),view=svg[0].getAttributeNS(null,"viewBox"),arr=view.split(" "),svgObj={urlParams:{filename:""},name:"",format:"",original_key:"",sourceFilename:"",svg_key:"",png_key:"",category:[],uploadId:null,initialScale:1,transformations:[],matrix:{},beingEdited:!1,origin:"Native",type:"Art",bbox:{x:parseInt(arr[0]),y:parseInt(arr[1]),width:parseInt(arr[2]),height:parseInt(arr[3])},filename:"","preserve-colors":!1,fillColor:{id:k,value:vm.left_design[k].color,origin:null,heather:!1,texture:""},outlineColor:{id:"default"+k,value:"#000000",origin:null,heather:!1,texture:""},newDesignData:{transform:{}}};"svg"===vm.left_design[k].type&&(svgObj.colors=_obj.colors),layers.push(svgObj)}}console.log("Left"),svgParent2=svgParent2.outerHTML,svgParent2=svgParent2.replace(/\r?\n|\r/g," "),svgParent2=svgParent2.replace(/<!--(.*?)-->/gi,""),svgParent2=svgParent2.replace(/\t/g,""),console.log(svgParent2);var left={removeLayerBinding:{},name:"left",layers:layers,colors:vm.total_left_colors,bbox:{x:parseInt(arr[0]),y:parseInt(arr[1]),width:parseInt(arr[2]),height:parseInt(arr[3])},absoluteBBox:{x:parseInt(arr[0]),y:parseInt(arr[1]),width:parseInt(arr[2]),height:parseInt(arr[3])},beingViewed:!1,initialFreeTransformAttrs:null,initialPrintableArea:null,referencePoint:{},ppi:18,editable:null,sequence:2,scaleAndMove:null,rasterImageInfo:[],svg:svgParent2};sides.push(left)}if(3==i){var svgParent3=document.createElementNS("http://www.w3.org/2000/svg","svg");svgParent3.setAttribute("style","border: 1px solid black"),svgParent3.setAttribute("width","189"),svgParent3.setAttribute("height","313"),svgParent3.setAttributeNS("http://www.w3.org/2000/xmlns/","xmlns:xlink","http://www.w3.org/1999/xlink"),svgParent3.setAttributeNS("http://www.w3.org/2000/xmlns/","xmlns","http://www.w3.org/2000/svg"),svgParent3.setAttributeNS(null,"viewBox","0 0 189 313");var foriegnObject=document.createElementNS("http://www.w3.org/2000/svg","foreignObject");foriegnObject.setAttribute("width","189"),foriegnObject.setAttribute("height","313");var mainDiv=document.createElement("div");mainDiv.setAttributeNS("http://www.w3.org/2000/xmlns/","xmlns","http://www.w3.org/1999/xhtml");var printDiv=document.createElement("div");printDiv.setAttribute("style","width: 189px;height:313px;overflow:hidden"),mainDiv.appendChild(printDiv);for(var layers=[],j=0;j<vm.right_design.length;j++){var art=$(".right").clone();art.find(".zoom-view,.visual-text,ul").remove(),art.css("position","relative").removeAttr("class").removeAttr("ng-repeat").removeAttr("id"),art.find(".user-box").css("position","absolute").removeAttr("on-drag-end").removeAttr("ng-style").removeAttr("id").removeAttr("class").removeAttr("ng-class").removeAttr("on-drag"),art.find("svg").removeAttr("ng-show").removeAttr("ng-if").removeAttr("ng-width").removeAttr("ng-height"),art.find("image").removeAttr("ng-click").removeAttr("ng-width").removeAttr("ng-height"),"img"!==vm.right_design[j].type&&art.find(".svgimg").remove(),printDiv.appendChild(art[j])}foriegnObject.appendChild(mainDiv),svgParent3.appendChild(foriegnObject),console.log(svgParent3);for(var k=0;k<vm.right_design.length;k++){if("text"===vm.right_design[k].type){var svgText,obj_=vm.right_design[k],element=$("#box"+vm.right_design[k].z_index+"right"),svg=element.children("svg")[0],view=svg.getAttributeNS(null,"viewBox"),arr=view.split(" ");svgText=$("#box"+vm.right_design[k].z_index+"right text").clone()[0],console.log(svgText),svgText.children[0].setAttributeNS(null,"x",parseInt(element.css("left"))),svgText.children[0].setAttributeNS(null,"y",parseInt(element.css("top"))),svgText.children[0].setAttributeNS(null,"width",parseInt(element.css("width"))),svgText.children[0].setAttributeNS(null,"height",parseInt(element.css("height")));var textObj={string:vm.right_design[k].text,font:{id:k,family:vm.right_design[k].font,filename:vm.right_design[k].font,tags:[],priority:1,url:null},fontSize:24,tag:"Popular",lineHeight:1.2,alignment:"center",colors:obj_.color,outlineThickness:0,transformations:[],matrix:{},beingEdited:!1,origin:"Native",type:"Text",bbox:{x:parseInt(arr[0]),y:parseInt(arr[1]),width:parseInt(arr[2]),height:parseInt(arr[3])},filename:"","preserve-colors":!1,fillColor:{id:k,value:vm.right_design[k].color,origin:null,heather:!1,texture:""},outlineColor:{id:"default"+k,value:"#000000",origin:null,heather:!1,texture:""},newDesignData:{transform:{}}};layers.push(textObj)}if("svg"===vm.right_design[k].type||"img"===vm.right_design[k].type){var svgText,element=$("#box"+vm.right_design[k].z_index+"right");svgText=element.find("image").clone()[0],svgText.setAttributeNS(null,"x",parseInt(element.css("left"))),svgText.setAttributeNS(null,"y",parseInt(element.css("top"))),svgText.setAttributeNS(null,"width",parseInt($("#box"+vm.right_design[k].z_index+"right svg").css("width"))),svgText.setAttributeNS(null,"height",parseInt($("#box"+vm.right_design[k].z_index+"right svg").css("height")));var obj_=vm.right_design[k],_obj=_.findWhere(vm.graphics,{id:obj_.id}),element=$("#box"+vm.right_design[k].z_index+"right"),svg=element.find("svg"),view=svg[0].getAttributeNS(null,"viewBox"),arr=view.split(" "),svgObj={urlParams:{filename:""},name:"",format:"",original_key:"",sourceFilename:"",svg_key:"",png_key:"",category:[],uploadId:null,initialScale:1,transformations:[],matrix:{},beingEdited:!1,origin:"Native",type:"Art",bbox:{x:parseInt(arr[0]),y:parseInt(arr[1]),width:parseInt(arr[2]),height:parseInt(arr[3])},filename:"","preserve-colors":!1,fillColor:{id:k,value:vm.right_design[k].color,origin:null,heather:!1,texture:""},outlineColor:{id:"default"+k,value:"#000000",origin:null,heather:!1,texture:""},newDesignData:{transform:{}}};"svg"===vm.right_design[k].type&&(svgObj.colors=_obj.colors),layers.push(svgObj)}}console.log("Right"),svgParent3=svgParent3.outerHTML,svgParent3=svgParent3.replace(/\r?\n|\r/g," "),svgParent3=svgParent3.replace(/<!--(.*?)-->/gi,""),svgParent3=svgParent3.replace(/\t/g,""),console.log(svgParent3);var right={removeLayerBinding:{},name:"right",layers:layers,colors:vm.total_right_colors,bbox:{x:parseInt(arr[0]),y:parseInt(arr[1]),width:parseInt(arr[2]),height:parseInt(arr[3])},absoluteBBox:{x:parseInt(arr[0]),y:parseInt(arr[1]),width:parseInt(arr[2]),height:parseInt(arr[3])},beingViewed:!1,initialFreeTransformAttrs:null,initialPrintableArea:null,referencePoint:{},ppi:18,editable:null,sequence:2,scaleAndMove:null,rasterImageInfo:[],svg:svgParent3};sides.push(right)}}}var design={lookupId:"f2345",sides:sides,activeSide:activeSide,high_quality_artwork:!0,crap_quality_artwork:!1,product_type_id:parseInt(vm.selectedShirt.id),frontColors:vm.total_front_colors.length,flashFront:!1,backColors:vm.total_back_colors.length,flashBack:!1,leftColors:vm.total_left_colors.length,flashLeft:!1,rightColors:vm.total_right_colors.length,flashRight:!1};console.log(sides),compaign_json.design=JSON.stringify(design),compaign_json.priorPrintableArea=JSON.stringify({front:"152,100,220,330",back:"155,60,215,330",left:"155,60,215,330",right:"155,60,215,330"}),DashboardService.LaunchCompaign(compaign_json).then(function(data){return 210==data.code?void $scope.show_login_form():(200==data.code?window.location.href=serverURL+data.data.campaign:211==data.code&&alert(data.message),void console.log(data))})}function build_url(){return vm.url=vm.compaign_title.replace("/","-"),vm.url=vm.url.replace(/\s/g,""),vm.url=vm.url.replace(/[^a-zA-Z0-9]/g,"-").toLowerCase(),"-"==vm.url.charAt(vm.url.length-1)?void(vm.url=vm.url.substring(0,vm.url.length-1)):void validate_url()}function validate_url(){return vm.url.length?(vm.url=vm.url.replace(/[^a-zA-Z0-9]/g,"-"),"-"==vm.url.charAt(vm.url.length-1)?void(vm.url=vm.url.substring(0,vm.url.length-1)):(vm.is_loading_url=!0,void setTimeout(function(){DashboardService.ValidateURL(vm.url).then(function(data){vm.is_loading_url=!1,data.data.available?vm.url_available=!0:vm.url_available=!1})},500))):void(vm.url_available=!1)}function remove_tag(index){vm.tags.splice(index,1)}function extract_tags(e){if(vm.raw_tags){if(vm.tags.length>4)return void alert("Max 5 tags are allowed");","==vm.raw_tags.slice(-1)&&(vm.tags.push(vm.raw_tags.substr(0,vm.raw_tags.length-1)),vm.raw_tags=""),13==e.keyCode&&(vm.tags.push(vm.raw_tags.substr(0,vm.raw_tags.length)),vm.raw_tags=""),vm.tags=_.uniq(vm.tags)}}function select_extra_shirt_color(color,deselect_others){if(color.selected&&vm.selectedModalShirt.id==vm.selectedShirt.id){var selected_colors_num=_.where(vm.selectedModalShirt.colors_available,{selected:!0}).length;if(1==selected_colors_num)return}color.selected?(color.selected=!1,vm.total_color_selected--,vm.limit_full=!1,vm.calculate_total_shirt_cost(vm.selectedModalShirt)):vm.total_color_selected<=14?(vm.limit_full=!1,color.selected=!0,vm.total_color_selected++):vm.limit_full=!0;var sould_be_selected=_.where(vm.selectedModalShirt.colors_available,{selected:!0}).length;sould_be_selected?vm.selectedModalShirt.selected=!0:vm.selectedModalShirt.selected=!1}function select_extra_shirt_colors(shirt){if(shirt.selected||15!=vm.total_color_selected){if(shirt.id==vm.selectedShirt.id)return shirt.selected=!0,void(shirt.colors_available[0].selected=!0);if(vm.selectedModalShirt=shirt,shirt.selected=!shirt.selected,!shirt.selected){var count=_.where(shirt.colors_available,{selected:!0}).length;return vm.total_color_selected-=count,void(vm.total_color_selected<0&&(vm.total_color_selected=0))}var count=_.where(shirt.colors_available,{selected:!0}).length;if(vm.total_color_selected+count>15)for(var i=0;i<shirt.colors_available.length;i++){var color=shirt.colors_available[i];color.selected=!1}shirt.colors_available[0].selected=!0,count=_.where(shirt.colors_available,{selected:!0}).length,vm.total_color_selected+=count;var sould_be_selected=_.where(vm.selectedModalShirt.colors_available,{selected:!0}).length;sould_be_selected||(vm.selectedModalShirt.selected=!1)}}function prepare_shirts(){}function is_available(shirt){return!0}function rotate_goal(arg){$timeout(function(){(vm.goal_target<vm.goal_range_min||vm.goal_target>vm.goal_range_max)&&(vm.goal_target<vm.goal_range_min?vm.goal_target=vm.goal_range_min:vm.goal_target=vm.goal_range_max,$scope.$$phase||$scope.$apply()),vm.goal_target||(vm.goal_target=vm.goal_range_min,$scope.$$phase||$scope.$apply()),$scope.$apply(function(){if(arg||(vm.goal_target=Number($("#goal_number").val())),Number(vm.goal_target)>vm.goal_range_max||Number(vm.goal_target)<vm.goal_range_min)return void(vm.invalid_goal=!0);vm.invalid_goal=!1;var degrees=Number($("#goal_number").val());degrees=arg?360/vm.goal_range_max*arg:360/vm.goal_range_max*degrees;var element=$("#goal_handler");element.css("transition","-webkit-transform 800ms ease"),element.css("-webkit-transform","rotate("+degrees+"deg)")})},500)}function setColor(color){var code_obj=_.findWhere(vm.colors,{hex:color.substr(1,color.length)}),selected_shirt=_.findWhere(vm.shirts_in_styles,{id:vm.selectedShirt.id});_.each(selected_shirt.colors_available,function(color_extra){color_extra.selected=!1});var modal_obj=_.findWhere(selected_shirt.colors_available,{color_id:code_obj.id});modal_obj.selected=!0,vm.shirt_color=color}function add_style(){viewOptionsModal.$promise.then(viewOptionsModal.show)}function getRandomInAnimations(){return"animated "+AnimationService.GetRandomInAnimation()}function show_toolbar(){$(".ta-toolbar").css("display","inline-block"),$(".ta-toolbar").addClass("animated fadeInDown")}function hide_toolbar(){$(".ta-toolbar").css("display","none")}function alert_invalid(arg){var animation=AnimationService.GetRandomInAnimation();$("."+arg).addClass("animated "+animation).one("webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend",function(){$(this).removeClass("animated "+animation)})}function loadCategories(){DashboardService.GetCategories().then(function(data){vm.categories=data.data})}function init(){$scope.serverURL=serverURL,vm.default_tab=1,vm.default_design_tab=1,vm.designSelected=!1,loadCategories(),vm.tags=[],vm.time_array=[];for(var i=1;i<=15;i++)vm.time_array.push(addDays(i+6));vm.imageURL=imageURL,vm.graphicsURL=graphicsURL,loadShirtStyle(),loadGraphics(),googleFonts(),init_shirt_drawings(),is_zoomed=!1,vm.shirt_color={},getColors(),vm.filtered_colors=[],vm.graphics=[],loadConfig(),vm.base_cost={},vm.total_front_colors=[],vm.total_back_colors=[],vm.total_left_colors=[],vm.total_right_colors=[],vm.total_shirt_colors=0,vm.is_searching=!1,vm.search_results=[],vm.search_page=1,init_predefined_categories(),vm.selected_text={},vm.selected_text.color="#000000",vm.is_loading_graphics=!1,search_query=""}function init_predefined_categories(){vm.predefined_categories=["Events","Days","Plants","Food","Drink","Nature","Patterns","Military","Transportation","Christmas","Earth","Day","Bees","Flowers","Rice","Beer","Spring","Floral","Air","force","Car","Easter","Mother's","Day","Cat","Leaf","Pizza","Soda","Summer","Tile","Army","Bike","Birthday","Father's","Day","Dog","Tree","Burgers","Tea","Autumn","Flowers","Bomb","Truck","New","year","Valentine","Day","Deer","Grass","Ice","Cream","Coffee","Winter","Tribal","Navy","Cycle","Anniversary","Cancer","Day","Dolphin","Wheat","Pasta","Milk","Camping","Aztec","Helicopter","Airplane","Ramadan","Independence","Day","Horse","Organic","Salad","Cocktail","Hunting","Chevron","Gun","Train","Diwali","Labor","Day","Birds","Sandwich","Whiskey","Fishing","Tanks","Boat","Party","Water","Day","Lion","Juice","Hiking","Medals","Scooter","Halloween","Poetry","Day","Tigers","Wine","Ocean","Soldiers","Taxi","Eid","Colors","Day","Eagle","Fire","Navy","Ships","Bus","Wedding","Health","Day","Monkey","Clouds","Spaceship","Festival","Family","Day","Parrots","Circus","Tourism","Day","Fish","Olympic","Games","International","Day","Owl","Friendship","Day","English","Language","Day","Religion","Sports","Elements","Designs","People","Entertainments","Fundraise","Islam","Football","Abstract","Arts","Family","Movies","Freedom","Speech","Jesus","Hockey","Textures","Illustrations","Child","Music","Human","Rights","Cross","Cycling","Flags","Emoji","Boy","Games","Youth","Temple","Soccer","Arrows","Cartoons","Girl","Comedy","/","Funny","Animal","Rescue","Torah","Basketball","Characters","Man","Social","Media","Hunger","Church","Golf","Typography","Woman","Culture","World","Peace","Star","of","David","Cricket","Mask","Body","Angels","Snowboarding","Crown","Baby","Hindu","Skateboarding","Love","Archery","King","Surfing","Queen","Place","Monuments","Technology","Computers","Telephones","Steres","Televisions","Mobile","Phones","abc-bones-filled","abc-bones-stoke","adventure-and-travel-color","airplanes","airports-icons","american-football","animal-compilation","animal-icon-02","animals","arrow-02","autumn","babe-icon-set","baby","baby-02","back-to-school","beauty","beauty-spa","beliefs-symbols","birds-species","birthday-party","brazil","browsers","business","business-and-office","business-seo","camping","carootns","cars","castles","category_list","cat-set","character-02","charity","chemistry","china","christmas","cinema","circle-medicine","circles-hospital-color","circus","city-element-02","clothes-and-footwear","clothes-stroke","cloud-02","color-seo","communication","computer-security","construction","country-flags","currencies","cute-animal","design-tools","devices","dialogue-assets","dinosaur","disco-music","ecommerce","education","electronics","emoticons","essential","expressions","fairy-tale-set","farm-element","file-type-and-content","file-types","filled-feathers","filled-management","finance","flags","flamenco-dance","flowers","food","food-and-restaurant","forest-animals","fruit-and-vegetable-juice","fruits-pack","furniture-and-decoration","furniture-and-household","game-assets","gardening","gastronomy-set","glypho","hairdressing","hand-and-gesture-02","hand-drawn-toys","hawcons-emoji-stroke","healthy-lifestyle","heartbeat","high-school","horses-2","hospital","human-body","interaction-assets","interface","ios7-set-lined-2","keyholes","kids","kids-avatars","landmarks","landscapes","logogram","logotypes","marketing","medals","medical","medicine-color","miscellaneous","moon-phase","multimedia-02","music-02","musical-genres","my-town-public-buildings","nature-flat-color","new-business","news","office","peace","people-culture","photo-02","picnic-and-bbq-icons","pokemon-go","pregnancy","printing","productivity-02","professions-avatars","rating-and-vadilation-set","real-estate","religion","sacred-geometry","school-02","science","science-icons","scientifics-study","sea-life","secret-service","security","seo-and-online-marketing","settings-and-display-set","shopping","sleep-time","snowflakes-set","social-icons-rounded","social-media","space","sports","sports-flat-color","star-set","startups-and-new-business","summer-fruits","summer-holiday","summertime-02","summertime-vacations","sun-icon-02","symbol","tatoo","technology","the-ultimate","toys","transportation","travel","travel-and-tourism","travel-and-transportation","trohies","ui-02","universe","user-interface","valentines","water","weather","web-buttons","web-design","web-interface","web-navigation-line-craft","wedding","yoga-poses","young-avatar","zodiac"];
}function calculate_front(){vm.total_front_colors=[];return vm.front_design.length?void angular.forEach(vm.front_design,function(obj,count){if("svg"==obj.type){var original=_.where(vm.graphics,{id:obj.id})[0];if(original.colors){var colors_array=original.colors.split(",");add_to_colors(1,colors_array)}}else if("img"==obj.type)add_to_colors(1,obj.colors);else if("text"==obj.type){var arr=[];arr[0]=obj.color,add_to_colors(1,arr)}}):void(vm.total_front_colors=[])}function calculate_back(){vm.total_back_colors=[];return vm.back_design.length?void angular.forEach(vm.back_design,function(obj,count){if("svg"==obj.type){var original=_.where(vm.graphics,{id:obj.id})[0];if(original.colors){var colors_array=original.colors.split(",");add_to_colors(2,colors_array)}}else if("img"==obj.type)add_to_colors(2,obj.colors);else if("text"==obj.type){var arr=[];arr[0]=obj.color,add_to_colors(2,arr)}}):void(vm.total_back_colors=[])}function calculate_left(){vm.total_left_colors=[];return vm.left_design.length?void angular.forEach(vm.left_design,function(obj,count){if("svg"==obj.type){var original=_.where(vm.graphics,{id:obj.id})[0];if(original.colors){var colors_array=original.colors.split(",");add_to_colors(3,colors_array)}}else if("img"==obj.type)add_to_colors(3,obj.colors);else if("text"==obj.type){var arr=[];arr[0]=obj.color,add_to_colors(3,arr)}}):void(vm.total_left_colors=[])}function calculate_right(){vm.total_right_colors=[];return vm.right_design.length?void angular.forEach(vm.right_design,function(obj,count){if("svg"==obj.type){var original=_.where(vm.graphics,{id:obj.id})[0];if(original.colors){var colors_array=original.colors.split(",");add_to_colors(4,colors_array)}}else if("img"==obj.type)add_to_colors(4,obj.colors);else if("text"==obj.type){var arr=[];arr[0]=obj.color,add_to_colors(4,arr)}}):void(vm.total_right_colors=[])}function calculate_total_color_cost(arg){var front=vm.total_front_colors.length>10?10:vm.total_front_colors.length,back=vm.total_back_colors.length>10?10:vm.total_back_colors.length,left=vm.total_left_colors.length>5?5:vm.total_left_colors.length,right=vm.total_right_colors.length>5?5:vm.total_right_colors.length;if(arg&&1!=arg.left&&(left=0,right=0),vm.selectedShirt&&(arg||1==vm.selectedShirt.left||(left=0,right=0,1!=vm.selectedShirt.back&&(back=0))),0==front)var front_price=0;else var front_price=.5*(front-1)+1.5;if(0==back)var back_price=0;else var back_price=.5*(back-1)+1.5;var left_price=.5*(left-1)+.5,right_price=.5*(right-1)+.5;arg?arg.total_color_cost=front_price+back_price+left_price+right_price:vm.total_color_cost=front_price+back_price+left_price+right_price}function calculate_unit_cost(arg){if(arg){for(var is_color=!1,index=0,i=0;i<arg.colors_available.length;i++){var color=arg.colors_available[i];if(color.selected&&Number(color.color_id)>1){is_color=!0,index=i;break}}return parseFloat(arg.colors_available[index].blank_cost)}var is_color=!1;if("#FFFFFF"!=vm.shirt_color&&(is_color=!0),vm.selectedShirt)return is_color?parseFloat(vm.selectedShirt.colors_available[1].blank_cost):parseFloat(vm.selectedShirt.colors_available[0].blank_cost)}function calculate_garment_discount(){return vm.goal_target>=50&&vm.goal_target<=99?5:vm.goal_target>=100&&vm.goal_target<=199?6:vm.goal_target>=200&&vm.goal_target<=499?7:vm.goal_target>=500&&vm.goal_target<=999?8:vm.goal_target>=1e3?10:0}function calculate_print_discount(){return vm.goal_target>=100&&vm.goal_target<=199?12:vm.goal_target>=200&&vm.goal_target<=499?14:vm.goal_target>=500&&vm.goal_target<=999?16:vm.goal_target>=1e3?20:0}function add_to_colors(side,colors){1==side&&(vm.total_front_colors=vm.total_front_colors.concat(colors),vm.total_front_colors=_.uniq(vm.total_front_colors)),2==side&&(vm.total_back_colors=vm.total_back_colors.concat(colors),vm.total_back_colors=_.uniq(vm.total_back_colors)),3==side&&(vm.total_left_colors=vm.total_left_colors.concat(colors),vm.total_left_colors=_.uniq(vm.total_left_colors)),4==side&&(vm.total_right_colors=vm.total_right_colors.concat(colors),vm.total_right_colors=_.uniq(vm.total_right_colors))}function getColors(){DashboardService.GetColors().then(function(data){vm.colors=data.data})}function zoom(){vm.is_zoomed=!vm.is_zoomed;var elem=$(".shirt-view"),detailLIst=$(".detail-list"),changeZoomIn=$(".icon-zoom-01"),changeZoomOut=$(".glyphicon-zoom-out");vm.is_zoomed?(elem.addClass("animated zoomPrint").one("webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend",function(){$(this).removeClass("animated zoomPrint")}),detailLIst.addClass("detailListZoom"),changeZoomIn.removeClass("icon-zoom-01").addClass("glyphicon glyphicon-zoom-out")):(elem.removeClass("zoomPrint animated"),detailLIst.removeClass("detailListZoom"),$(changeZoomOut).removeClass("glyphicon glyphicon-zoom-out").addClass("icon-zoom-01"))}function googleFonts(){vm.fonts=DashboardService.GetGoogleFonts(),angular.forEach(vm.fonts,function(font){vm.fontsTag.push(font.category)}),vm.uniqFontTags=_.uniq(vm.fontsTag)}function loadShirtStyle(){DashboardService.GetShirtStyle().then(function(result){result&&result.data&&(vm.shirts=result.data,_.each(vm.shirts,function(shirt){var reg=/(.*)\.[^.]+$/,front_thumb=reg.exec(shirt.image_front_url);shirt.front_small=front_thumb[1]+"_small.png"}),vm.shirts_in_styles=angular.copy(vm.shirts),_.each(vm.shirts_in_styles,function(style){style.selected=!1,_.each(style.colors_available,function(color){color.selected=!1,_.each(vm.colors,function(_color){_color.id==color.color_id&&(color.color="#"+_color.hex)})})}),selectShirt(vm.shirts[0]),$scope.get_style(),vm.filtered_colors=[],setTimeout(function(){for(var i=0;i<vm.colors.length;i++){var color=vm.colors[i],id=vm.selectedShirt.colors_available.filter(function(_color){return _color.color_id==color.id});id&&vm.filtered_colors.push("#"+color.hex),vm.shirt_color=vm.filtered_colors[0]}},500))})}function loadGraphics(){vm.search_graphics="",DashboardService.GetGraphics().then(function(result){result&&result.data&&(vm.graphics=vm.graphics.concat(result.data))})}function loadConfig(exclude_goals){DashboardService.GetConfig().then(function(data){vm.user={},vm.user.pages={},vm.config=data.data,exclude_goals||(vm.goal_range_max=Number(vm.config.config.goal_range_max),vm.goal_range_min=Number(vm.config.config.goal_range_min),vm.goal_target=Number(vm.goal_range_min),rotate_goal(vm.goal_range_min)),vm.footer_pages=vm.config.pages,1==vm.config.login.status&&(vm.user.id=vm.config.login.id,vm.user.name=vm.config.login.name,vm.user.pages=vm.config.login.pages)})}function addDays(number_of_days){var d=new Date,d2=d.setDate(d.getDate()+number_of_days);return new Date(d2)}function reset_style_shirts(){for(var i=0;i<vm.shirts_in_styles.length;i++){for(var j=0;j<vm.shirts_in_styles[i].colors_available.length;j++)vm.shirts_in_styles[i].colors_available[j].selected=!1;vm.shirts_in_styles[i].selected-!1}vm.total_color_selected=1}function selectShirt(shirt){reset_style_shirts(),vm.default_tab=1,vm.selectedShirt=shirt,vm.filtered_colors=[];var reg=/(.*)\.[^.]+$/,front_thumb=reg.exec(shirt.image_front_url),back_thumb=reg.exec(shirt.image_back_url),left_thumb=reg.exec(shirt.image_left_url),right_thumb=reg.exec(shirt.image_right_url);vm.selectedShirSide={front:front_thumb[1]+"_small.png",back:back_thumb[1]+"_small.png"},1==shirt.id?(vm.isSleeve=!0,vm.selectedShirSide.left=left_thumb[1]+"_small.png",vm.selectedShirSide.right=right_thumb[1]+"_small.png"):vm.isSleeve=!1,console.log(vm.selectedShirSide),angular.forEach(vm.colors,function(color,count){var id=vm.selectedShirt.colors_available.filter(function(_color){return _color.color_id==color.id});id[0]&&vm.filtered_colors.push("#"+color.hex),vm.shirt_color=vm.filtered_colors[0],0==count&&_.each(vm.shirts_in_styles,function(shirt){shirt.id==vm.selectedShirt.id?shirt.selected=!0:shirt.selected=!1,setColor(vm.filtered_colors[0])})}),$scope.get_style()}function updateSize($w,$h,index,side){var currentSide="";1===side?currentSide="front":2===side?currentSide="back":3===side?currentSide="left":4===side&&(currentSide="right");var e=$("#box"+index+currentSide);svg=e.find("svg"),view=svg[0].getAttributeNS(null,"viewBox"),width=svg[0].getAttributeNS(null,"width"),height=svg[0].getAttributeNS(null,"height"),view=view.split(" "),svg[0].setAttributeNS(null,"width",$w),svg[0].setAttributeNS(null,"height",$h),svg[0].setAttributeNS(null,"viewBox","0 0 "+$w*view[2]/width+" "+$h*view[3]/height),$(e).css({width:$w+"px",height:$h+"px"})}function selectGraphic(graphic,is_searched){if(is_searched&&vm.graphics.push(graphic),1===vm.default_tab){var z_index=1;z_index=vm.front_design.length?vm.front_design[vm.front_design.length-1].z_index+1:1;var obj={type:"svg",src:vm.graphicsURL+graphic.filename,z_index:z_index,width:graphic.width+"px",height:graphic.height+"px",id:graphic.id};vm.front_design.push(obj),_.each(vm.front_design,function(front){front.z_index!=z_index&&($("#tools"+front.z_index+"front").css({visibility:"hidden"}),$("#box"+front.z_index+"front").removeClass("border"),front.selected=!1)}),setTimeout(function(){var currentSvg=document.querySelectorAll("#box"+z_index+"front svg");currentSvg[0].setAttributeNS(null,"viewBox","0 20 "+graphic.width+" "+graphic.height),$("#tools"+z_index+"front").css({visibility:"visible"}),$("#box"+z_index+"front").addClass("border"),vm.select_text(z_index),console.log(z_index)},500),vm.selectedGraphic=graphic,vm.designSelected=!0}else if(2===vm.default_tab){var z_index=1;z_index=vm.back_design.length?vm.back_design[vm.back_design.length-1].z_index+1:1;var obj={type:"svg",src:vm.graphicsURL+graphic.filename,z_index:z_index,width:graphic.width+"px",height:graphic.height+"px",id:graphic.id};vm.back_design.push(obj),_.each(vm.back_design,function(back){back.z_index!=z_index&&($("#tools"+back.z_index+"back").css({visibility:"hidden"}),$("#box"+back.z_index+"back").removeClass("border"),back.selected=!1)}),setTimeout(function(){var currentSvg=document.querySelectorAll("#box"+z_index+"back svg");console.log(currentSvg[0]),currentSvg[0].setAttributeNS(null,"viewBox","0 20 "+graphic.width+" "+graphic.height),$("#tools"+z_index+"back").css({visibility:"visible"}),$("#box"+z_index+"back").addClass("border"),vm.select_text(z_index)},500),vm.selectedGraphic=graphic,vm.designSelected=!0}else if(3===vm.default_tab){var z_index=1;z_index=vm.left_design.length?vm.left_design[vm.left_design.length-1].z_index+1:1;var obj={type:"svg",src:vm.graphicsURL+graphic.filename,z_index:z_index,width:graphic.width+"px",height:graphic.height+"px",id:graphic.id};vm.left_design.push(obj),_.each(vm.left_design,function(back){back.z_index!=z_index&&($("#tools"+back.z_index+"left").css({visibility:"hidden"}),$("#box"+back.z_index+"left").removeClass("border"),back.selected=!1)}),setTimeout(function(){var currentSvg=document.querySelectorAll("#box"+z_index+"left svg");console.log(currentSvg[0]),currentSvg[0].setAttributeNS(null,"viewBox","0 20 "+graphic.width+" "+graphic.height),$("#tools"+z_index+"left").css({visibility:"visible"}),$("#box"+z_index+"left").addClass("border"),vm.select_text(z_index)},500),vm.selectedGraphic=graphic,vm.designSelected=!0}else if(4===vm.default_tab){var z_index=1;z_index=vm.right_design.length?vm.right_design[vm.right_design.length-1].z_index+1:1;var obj={type:"svg",src:vm.graphicsURL+graphic.filename,z_index:z_index,width:graphic.width+"px",height:graphic.height+"px",id:graphic.id};vm.right_design.push(obj),_.each(vm.right_design,function(right){right.z_index!=z_index&&($("#tools"+right.z_index+"right").css({visibility:"hidden"}),$("#box"+right.z_index+"right").removeClass("border"),right.selected=!1)}),setTimeout(function(){var currentSvg=document.querySelectorAll("#box"+z_index+"right svg");console.log(currentSvg[0]),currentSvg[0].setAttributeNS(null,"viewBox","0 20 "+graphic.width+" "+graphic.height),$("#tools"+z_index+"right").css({visibility:"visible"}),$("#box"+z_index+"right").addClass("border"),vm.select_text(z_index)},500),vm.selectedGraphic=graphic,vm.designSelected=!0}}function set_design_tab(arg){vm.default_design_tab=arg,3==vm.default_design_tab,3!==vm.default_design_tab&&4!==vm.default_design_tab||(angular.forEach(intervals,function(interval){$interval.cancel(interval)}),intervals.length=0)}function activate_tab(arg){vm.default_tab=arg,2===arg&&(vm.hide=!0),1===arg&&(angular.forEach(intervals,function(interval){$interval.cancel(interval)}),intervals.length=0,$(".printable-area").removeClass("crossing_border")),2!==arg&&3!==arg&&4!==arg||($(".user-list").css({visibility:"hidden"}),$(".user-box").removeClass("border"),angular.forEach(intervals,function(interval){$interval.cancel(interval)}),intervals.length=0,$(".printable-area").removeClass("crossing_border"))}function loadDefault(){}function init_shirt_drawings(){vm.front_design=[],vm.back_design=[],vm.left_design=[],vm.right_design=[]}$scope.image=null,$scope.imageFileName="";var vm=this;vm.loadDefault=loadDefault,vm.activate_tab=activate_tab,vm.set_design_tab=set_design_tab,vm.selectShirt=selectShirt,vm.selectGraphic=selectGraphic,vm.fontsURL=fontsURL,vm.selectedGraphic={},vm.rotation=!1,vm.goal_range_max=0,vm.goal_range_min=0,vm.goal_target=0,vm.fontsTag=[],vm.customImage="",vm.fontList={title:"Font List"},vm.fontSelected={family:"ABeeZee"},vm.rotation=!1,vm.hexPicker={color:""},vm.is_zoomed=!1,vm.other_apperals=[],vm.imageURL1=imageURL1,vm.show_toolbar=show_toolbar,vm.hide_toolbar=hide_toolbar,vm.alert_invalid=alert_invalid,vm.getRandomInAnimation=getRandomInAnimations,vm.rotate_goal=rotate_goal,vm.zoom=zoom,vm.setColor=setColor,vm.loadGraphics=loadGraphics,vm.add_style=add_style,vm.is_available=is_available,vm.prepare_shirts=prepare_shirts,vm.shirts_in_styles=[],vm.select_extra_shirt_colors=select_extra_shirt_colors,vm.total_color_selected=1,vm.select_extra_shirt_color=select_extra_shirt_color,vm.limit_full=!1,vm.profit_array=[],vm.extract_tags=extract_tags,vm.remove_tag=remove_tag,vm.build_url=build_url,vm.validate_url=validate_url,vm.launch_compaign=launch_compaign,vm.selectedProduct={},window.timeoutList=new Array,window.intervalList=new Array,window.oldSetTimeout=window.setTimeout,window.oldSetInterval=window.setInterval,window.oldClearTimeout=window.clearTimeout,window.oldClearInterval=window.clearInterval,window.setTimeout=function(code,delay){var retval=window.oldSetTimeout(code,delay);return window.timeoutList.push(retval),retval},window.clearTimeout=function(id){var ind=window.timeoutList.indexOf(id);ind>=0&&window.timeoutList.splice(ind,1);var retval=window.oldClearTimeout(id);return retval},window.setInterval=function(code,delay){var retval=window.oldSetInterval(code,delay);return window.intervalList.push(retval),retval},window.clearInterval=function(id){var ind=window.intervalList.indexOf(id);ind>=0&&window.intervalList.splice(ind,1);var retval=window.oldClearInterval(id);return retval},window.clearAllIntervals=function(){for(var i in window.intervalList)window.oldClearInterval(window.intervalList[i]);window.intervalList=new Array},vm.manage_colors=function(shirt){vm.manage_colors_bool=!0,vm.selectedModalShirt=shirt,angular.forEach(shirt.colors_available,function(color){for(var i=0;i<vm.colors.length;i++){var globalColor=vm.colors[i];globalColor.id==color.color_id&&(color.color="#"+globalColor.hex)}}),viewShirtColors.$promise.then(viewShirtColors.show)},vm.search_tees=function(){vm.tee_search_name&&window.location.assign(serverURL+"search?p=1&q="+vm.tee_search_name)};vm.register_user=function(){DashboardService.SignUp(vm.sign_up).then(function(data){200!=data.code?alert(data.message):(sign_up_modal.$promise.then(sign_up_modal.hide),DashboardService.GetConfig().then(function(data){vm.user={},vm.user.pages={},vm.config=data.data,1==vm.config.login.status&&(vm.user.id=vm.config.login.id,vm.user.name=vm.config.login.name,vm.user.pages=vm.config.login.pages)}),alert("Account created and you are now logged in!"))})},vm.show_shirts_modal=function(){vm.manage_colors_bool?vm.manage_colors_bool=!1:viewOptionsModal.$promise.then(viewOptionsModal.show),viewShirtColors.$promise.then(viewShirtColors.hide)},$scope.show_signUpForm=function(){loginModal.$promise.then(loginModal.hide),sign_up_modal.$promise.then(sign_up_modal.show)},$scope.show_login_form=function(){sign_up_modal.$promise.then(sign_up_modal.hide),loginModal.$promise.then(loginModal.show)};var loginModal=$modal({scope:$scope,animation:"am-fade-and-scale",placement:"center",template:"NgApp/Home/VIews/login_modal.html",show:!1,title:"Login with teedesign"}),sign_up_modal=$modal({scope:$scope,animation:"am-fade-and-scale",placement:"center",template:"NgApp/Home/VIews/sign_up_modal.html",show:!1,title:"Login with teedesign"});vm.login=function(){DashboardService.Login(vm.loginCred).then(function(data){200!=data.code?alert(data.message):(loginModal.$promise.then(loginModal.hide),DashboardService.GetConfig().then(function(data){vm.user={},vm.user.pages={},vm.config=data.data,1==vm.config.login.status&&(vm.user.id=vm.config.login.id,vm.user.name=vm.config.login.name,vm.user.pages=vm.config.login.pages)}))})},$scope.validateImage=function(node){},vm.deselectLayer=function(){var arg=vm.default_tab;1!==arg&&2!==arg&&3!==arg&&4!==arg||($(".user-list").css({visibility:"hidden"}),$(".user-box").removeClass("border"))},$scope.$watch("files",function(newVal,oldVal){$scope.upload($scope.files)}),$scope.$watch("file",function(newVal,oldVal){null!=$scope.file&&($scope.files=[$scope.file])}),$scope.log="",$scope.upload=function(files){if(!vm.agreed)return void vm.alert_invalid("custom-checkbox");if(files&&files.length){vm.is_uploading_file=!0;for(var i=0;i<files.length;i++){var file=files[i];file.$error||Upload.upload({url:baseURL+"/upload",data:{image:file}}).then(function(resp){if(vm.is_uploading_file=!1,200!=resp.data.code)return void alert(resp.data.data);if(vm.customImageObj=resp.data.data,vm.customImage=resp.data.data.path,1===vm.default_tab){var z_index=1;z_index=vm.front_design.length?vm.front_design[vm.front_design.length-1].z_index+1:1;var obj={type:"img",z_index:z_index,src:vm.customImage,colors:resp.data.data.colors};vm.front_design.push(obj)}if(2===vm.default_tab){var z_index=1;z_index=vm.back_design.length?vm.back_design[vm.back_design.length-1].z_index+1:1;var obj={type:"img",z_index:z_index,src:vm.customImage,colors:resp.data.data.colors};vm.back_design.push(obj)}if(3===vm.default_tab){var z_index=1;z_index=vm.left_design.length?vm.left_design[vm.left_design.length-1].z_index+1:1;var obj={type:"img",z_index:z_index,src:vm.customImage,colors:resp.data.data.colors};vm.left_design.push(obj)}else if(4===vm.default_tab){var z_index=1;z_index=vm.right_design.length?vm.right_design[vm.right_design.length-1].z_index+1:1;var obj={type:"img",z_index:z_index,src:vm.customImage,colors:resp.data.data.colors};vm.right_design.push(obj)}},null,function(evt){})}}};var viewOptionsModal=$modal({scope:$scope,animation:"am-fade-and-scale",placement:"center",template:"NgApp/Home/VIews/viewOptions.html",show:!1,title:"Select Product"}),viewShirtColors=$modal({scope:$scope,animation:"am-fade-and-scale",placement:"center",template:"NgApp/Home/VIews/setShirtColors.html",show:!1,title:"Select Colors"});$scope.class=function(){return"animated fadeInDown"},init(),vm.search_arts=function(){return vm.arts_not_found=!1,vm.search_page=1,vm.search_graphics&&0!=vm.search_graphics.trim().length?(search_query!=vm.search_graphics&&(vm.search_page=1,vm.search_results=[]),vm.is_searching=!0,vm.is_loading_graphics=!0,DashboardService.SearchArts(vm.search_graphics,vm.search_page).then(function(data){return data.data.length?(search_query=angular.copy(vm.search_graphics),vm.search_results=data.data,vm.search_page++,vm.arts_not_found=!1,void(vm.is_loading_graphics=!1)):(vm.arts_not_found=!0,void(vm.is_loading_graphics=!1))}),void 0):(vm.is_searching=!1,vm.search_page=1,void(vm.search_results=[]))},vm.search_more=function(){vm.arts_not_found=!1,vm.is_searching=!0,search_query!=vm.search_graphics&&(vm.search_page=1,vm.search_results=[]),vm.is_loading_graphics=!0,DashboardService.SearchArts(vm.search_graphics,vm.search_page).then(function(data){return vm.is_loading_graphics=!1,data.data.length?(vm.search_results=vm.search_results.concat(data.data),vm.search_page++,void(vm.arts_not_found=!1)):void(vm.arts_not_found=!0)})},vm.validate_profit=function(profit,shirt){$timeout(function(){profit&&profit<1&&(shirt.profit=1),shirt.profit||(shirt.profit=1)},100)},vm.validate_search=function(e){return e.preventDefault(),vm.search_graphics&&0!=vm.search_graphics.trim().length?void("13"==e.keyCode&&vm.search_arts()):(vm.is_searching=!1,void(vm.search_page=1))},$scope.$watch("vm.shirt_color",function(newVal,oldVal){if(newVal!=oldVal){var color_val=vm.shirt_color.substr(1,vm.shirt_color.length-1),_color_id=_.findWhere(vm.colors,{hex:color_val}).id;vm.base_cost.blank_cost=parseFloat(_.findWhere(vm.selectedShirt.colors_available,{color_id:_color_id}).blank_cost),vm.calculate_total_shirt_cost()}}),$scope.$watch("vm.selectedShirt",function(newVal,oldVal){newVal!=oldVal&&setTimeout(function(){var color_val=vm.shirt_color.substr(1,vm.shirt_color.length-1),_color_id=_.findWhere(vm.colors,{hex:color_val}).id;vm.base_cost.blank_cost=parseFloat(_.findWhere(vm.selectedShirt.colors_available,{color_id:_color_id}).blank_cost),vm.calculate_total_shirt_cost()},500)}),$scope.$watchCollection("vm.front_design",function(newVal,oldVal){if(newVal!=oldVal){angular.forEach(vm.front_design,function(obj,count){if("svg"==obj.type){var original=_.where(vm.graphics,{id:obj.id})[0];if(original.colors){var colors_array=original.colors.split(",");add_to_colors(1,colors_array)}}else if("img"==obj.type)add_to_colors(1,obj.colors);else if("text"==obj.type){var arr=[];arr[0]=obj.color,add_to_colors(1,arr)}})}},!0),$scope.$watch("vm.back_design",function(newVal,oldVal){if(newVal!=oldVal){angular.forEach(vm.back_design,function(obj,count){if("svg"==obj.type){var original=_.where(vm.graphics,{id:obj.id})[0];if(original.colors){var colors_array=original.colors.split(",");add_to_colors(2,colors_array)}}else if("img"==obj.type)add_to_colors(2,obj.colors);else if("text"==obj.type){var arr=[];arr[0]=obj.color,add_to_colors(2,arr)}})}},!0),$scope.$watch("vm.left_design",function(newVal,oldVal){if(newVal!=oldVal){angular.forEach(vm.left_design,function(obj,count){if("svg"==obj.type){var original=_.where(vm.graphics,{id:obj.id})[0];if(original.colors){var colors_array=original.colors.split(",");add_to_colors(3,colors_array)}}else if("img"==obj.type)add_to_colors(3,obj.colors);else if("text"==obj.type){var arr=[];arr[0]=obj.color,add_to_colors(3,arr)}})}},!0),$scope.$watch("vm.right_design",function(newVal,oldVal){if(newVal!=oldVal){angular.forEach(vm.right_design,function(obj,count){if("svg"==obj.type){var original=_.where(vm.graphics,{id:obj.id})[0];if(original.colors){var colors_array=original.colors.split(",");add_to_colors(4,colors_array)}}else if("img"==obj.type)add_to_colors(4,obj.colors);else if("text"==obj.type){var arr=[];arr[0]=obj.color,add_to_colors(2,arr)}})}},!0),$scope.$watch("vm.total_front_colors",function(newVal,oldVal){newVal!=oldVal&&(vm.total_shirt_colors=vm.total_front_colors.length+vm.total_back_colors.length+vm.total_left_colors.length+vm.total_right_colors.length,vm.calculate_total_shirt_cost())}),$scope.$watch("vm.total_back_colors",function(newVal,oldVal){newVal!=oldVal&&(vm.total_shirt_colors=vm.total_front_colors.length+vm.total_back_colors.length+vm.total_left_colors.length+vm.total_right_colors.length,vm.calculate_total_shirt_cost())}),$scope.$watch("vm.total_left_colors",function(newVal,oldVal){newVal!=oldVal&&(vm.total_shirt_colors=vm.total_front_colors.length+vm.total_back_colors.length+vm.total_left_colors.length+vm.total_right_colors.length,vm.calculate_total_shirt_cost())}),$scope.$watch("vm.total_right_colors",function(newVal,oldVal){newVal!=oldVal&&(vm.total_shirt_colors=vm.total_front_colors.length+vm.total_back_colors.length+vm.total_left_colors.length+vm.total_right_colors.length,vm.calculate_total_shirt_cost())}),$scope.$watch("vm.total_shirt_colors",function(newVal,oldVal){newVal!=oldVal&&vm.calculate_total_shirt_cost()}),$scope.$watch("vm.goal_target",function(newVal,oldVal){newVal!=oldVal&&setTimeout(function(){vm.calculate_total_shirt_cost()},500)}),$rootScope.$on("$stateChangeSuccess",function(event,toState,toParams,fromState,fromParams,options){window.scrollTo(0,0)}),vm.calculate_profit=function(){vm.shirts_in_styles&&vm.shirts_in_styles.length&&angular.forEach(vm.shirts_in_styles,function(shirt,index){if(shirt.selected){var profit=(_.where(vm.shirts_in_styles,{selected:!0}).length,shirt.total_shirt_base_cost/vm.goal_target,shirt.profit);shirt.total_profit=profit*vm.goal_target;var values=_.where(vm.shirts_in_styles,{selected:!0});values=_.sortBy(values,function(num){return num.total_profit}),vm.profits={},vm.profits.min=values[0].total_profit,vm.profits.max=values[values.length-1].total_profit,$scope.$$phase||$scope.$apply()}}),$scope.$$phase||$scope.$apply()},vm.calculate_total_shirt_cost=function(arg){if(arg){var unit_cost=calculate_unit_cost(arg);calculate_total_color_cost(arg);var total_colors_with_goal=arg.total_color_cost*vm.goal_target,print_percent=calculate_print_discount(),garment_discount=calculate_garment_discount(),color_price_with_discount=total_colors_with_goal-print_percent/100*total_colors_with_goal,total_garment_price=unit_cost*vm.goal_target,garment_price_with_discount=total_garment_price-garment_discount/100*total_garment_price;arg.total_shirt_base_cost=garment_price_with_discount+color_price_with_discount,arg.total_unit_cost=arg.total_shirt_base_cost/vm.goal_target}else{var unit_cost=calculate_unit_cost();calculate_total_color_cost();var total_colors_with_goal=vm.total_color_cost*vm.goal_target,print_percent=calculate_print_discount(),garment_discount=calculate_garment_discount(),color_price_with_discount=total_colors_with_goal-print_percent/100*total_colors_with_goal,total_garment_price=unit_cost*vm.goal_target,garment_price_with_discount=total_garment_price-garment_discount/100*total_garment_price;vm.total_shirt_base_cost=garment_price_with_discount+color_price_with_discount,vm.total_unit_cost=vm.total_shirt_base_cost/vm.goal_target,vm.shirts_in_styles&&vm.shirts_in_styles.length&&angular.forEach(vm.shirts_in_styles,function(shirt){vm.calculate_total_shirt_cost(shirt)})}$scope.$$phase||$scope.$apply(),vm.calculate_profit()},$scope.$watch("vm.selected_text.color",function(newVal,oldVal){if(oldVal){if(1==vm.default_tab){var index=_.findIndex(vm.total_front_colors,oldVal);vm.total_front_colors.splice(index,1)}if(2==vm.default_tab){var index=_.findIndex(vm.total_back_colors,oldVal);vm.total_back_colors.splice(index,1)}if(3==vm.default_tab){var index=_.findIndex(vm.total_left_colors,oldVal);vm.total_left_colors.splice(index,1)}if(4==vm.default_tab){var index=_.findIndex(vm.total_right_colors,oldVal);vm.total_right_colors.splice(index,1)}}if(1==vm.default_tab){for(var texts=_.where(vm.front_design,{type:"text"}),colors=[],i=0;i<texts.length;i++)colors.push(texts[i].color);add_to_colors(1,colors)}else if(2==vm.default_tab){for(var texts=_.where(vm.back_design,{type:"text"}),colors=[],i=0;i<texts.length;i++)colors.push(texts[i].color);add_to_colors(2,colors)}else if(3==vm.default_tab){for(var texts=_.where(vm.left_design,{type:"text"}),colors=[],i=0;i<texts.length;i++)colors.push(texts[i].color);add_to_colors(3,colors)}else if(4==vm.default_tab){for(var texts=_.where(vm.right_design,{type:"text"}),colors=[],i=0;i<texts.length;i++)colors.push(texts[i].color);add_to_colors(4,colors)}}),$scope.change=function(){if(1==vm.default_tab){for(var texts=_.where(vm.front_design,{type:"text"}),colors=[],i=0;i<texts.length;i++)colors.push(texts[i].color);add_to_colors(1,colors)}else if(2==vm.default_tab){var arr=[];arr[0]=vm.selected_text.color,add_to_colors(2,arr)}else if(3==vm.default_tab){var arr=[];arr[0]=vm.selected_text.color,add_to_colors(3,arr)}else if(4==vm.default_tab){var arr=[];arr[0]=vm.selected_text.color,add_to_colors(4,arr)}},$scope.$watch("vm.base_cost",function(newVal,oldVal){newVal!=oldVal&&(vm.base_cost.total=vm.base_cost.blank_cost)},!0),$scope.get_style=function(){vm.selectedShirt&&(1==vm.default_tab?($scope.temp_parent="top:"+vm.selectedShirt.printable_front_top+"px;left:"+vm.selectedShirt.printable_front_left+"px",$scope.temp_child="width:"+vm.selectedShirt.printable_front_width+"px;height:"+vm.selectedShirt.printable_front_height+"px"):2==vm.default_tab?($scope.temp_parent="top:"+vm.selectedShirt.printable_back_top+"px;left:"+vm.selectedShirt.printable_back_left+"px",$scope.temp_child="width:"+vm.selectedShirt.printable_back_width+"px;height:"+vm.selectedShirt.printable_back_height+"px"):3==vm.default_tab?($scope.temp_parent="top:"+vm.selectedShirt.printable_left_top+"px;left:"+vm.selectedShirt.printable_left_left+"px",$scope.temp_child="width:"+vm.selectedShirt.printable_left_width+"px;height:"+vm.selectedShirt.printable_left_height+"px"):4==vm.default_tab&&($scope.temp_parent="top:"+vm.selectedShirt.printable_right_top+"px;left:"+vm.selectedShirt.printable_right_left+"px",$scope.temp_child="width:"+vm.selectedShirt.printable_right_width+"px;height:"+vm.selectedShirt.printable_right_height+"px"))},vm.select_products=function(){viewOptionsModal.$promise.then(viewOptionsModal.show)},vm.hold_display=function(){vm.is_printable_area_visible=!1,setTimeout(function(){vm.is_printable_area_visible=!0,$scope.$$phase||$scope.$apply()},3e3)},vm.confirm_profit_calculated=function(){_.each(vm.shirts_in_styles,function(shirt){shirt.id==vm.selectedShirt.id?shirt.selected=!0:shirt.selected=!1,vm.shirt_color||setColor(vm.filtered_colors[0])})},vm.fontChanged=function(val){},vm.fontSelection=function(font,event){if(1===vm.default_tab){var selected=_.findWhere(vm.front_design,{selected:!0}),e=$("#box"+selected.z_index+"front"),txt=e.find("text"),size1=txt[0].getBoundingClientRect(),size2=e[0].getBoundingClientRect(),$w=1.3*parseInt(size1.width),$h=parseInt(size1.height);updateSize($w,$h,selected.z_index,1);var svg=e.find("svg"),view=svg[0].getAttributeNS(null,"viewBox"),arr=view.split(" "),y=txt[0].getAttributeNS(null,"y");y=Math.round(y)+Math.round(size2.top)-Math.round(size1.top)-(Math.round(size2.top)-Math.round(size1.top))*(($w-arr[2])/$w),txt[0].setAttributeNS(null,"y",y),selected.font=font.family}else if(2===vm.default_tab){var selected=_.findWhere(vm.back_design,{selected:!0}),e=$("#box"+selected.z_index+"back"),txt=e.find("text"),size1=txt[0].getBoundingClientRect(),size2=e[0].getBoundingClientRect(),$w=1.3*parseInt(size1.width),$h=parseInt(size1.height);updateSize($w,$h,selected.z_index,2);var svg=e.find("svg"),view=svg[0].getAttributeNS(null,"viewBox"),arr=view.split(" "),y=txt[0].getAttributeNS(null,"y");y=Math.round(y)+Math.round(size2.top)-Math.round(size1.top)-(Math.round(size2.top)-Math.round(size1.top))*(($w-arr[2])/$w),txt[0].setAttributeNS(null,"y",y),selected.font=font.family}else if(3===vm.default_tab){var selected=_.findWhere(vm.left_design,{selected:!0}),e=$("#box"+selected.z_index+"left"),txt=e.find("text"),size1=txt[0].getBoundingClientRect(),size2=e[0].getBoundingClientRect(),$w=1.3*parseInt(size1.width),$h=parseInt(size1.height);updateSize($w,$h,selected.z_index,3);var svg=e.find("svg"),view=svg[0].getAttributeNS(null,"viewBox"),arr=view.split(" "),y=txt[0].getAttributeNS(null,"y");y=Math.round(y)+Math.round(size2.top)-Math.round(size1.top)-(Math.round(size2.top)-Math.round(size1.top))*(($w-arr[2])/$w),txt[0].setAttributeNS(null,"y",y),selected.font=font.family}else if(4===vm.default_tab){var selected=_.findWhere(vm.right_design,{selected:!0}),e=$("#box"+selected.z_index+"right"),txt=e.find("text"),size1=txt[0].getBoundingClientRect(),size2=e[0].getBoundingClientRect(),$w=1.3*parseInt(size1.width),$h=parseInt(size1.height);updateSize($w,$h,selected.z_index,4);var svg=e.find("svg"),view=svg[0].getAttributeNS(null,"viewBox"),arr=view.split(" "),y=txt[0].getAttributeNS(null,"y");y=Math.round(y)+Math.round(size2.top)-Math.round(size1.top)-(Math.round(size2.top)-Math.round(size1.top))*(($w-arr[2])/$w),
txt[0].setAttributeNS(null,"y",y),selected.font=font.family}},$scope.trustSrc=function(src){return $sce.trustAsResourceUrl(src)},vm.deleteLayer=function(zindex){vm.front_design.length&&vm.back_design.length&&vm.left_design.length&&vm.right_design.length||(angular.forEach(intervals,function(interval){$interval.cancel(interval)}),intervals.length=0),$(".printable-area").hasClass("crossing_border")&&$(".printable-area").removeClass("crossing_border"),1===vm.default_tab?(vm.front_design=_.reject(vm.front_design,function(layer){return layer.z_index===zindex}),calculate_front()):2===vm.default_tab?(vm.back_design=_.reject(vm.back_design,function(layer){return layer.z_index===zindex}),calculate_back()):3===vm.default_tab?(vm.left_design=_.reject(vm.left_design,function(layer){return layer.z_index===zindex}),calculate_left()):4===vm.default_tab&&(vm.right_design=_.reject(vm.right_design,function(layer){return layer.z_index===zindex}),calculate_right())};var intervals=[];vm.select_text=function(index,e){if(1==vm.default_tab){_index=_.findIndex(vm.front_design,function(x){return x.z_index==index}),vm.selected_text=vm.front_design[_index],vm.default_design_tab="svg"===vm.selected_text.type?2:3;var params=(document.querySelector("#box"+vm.selected_text.z_index+"front tspan"),{start:function(event,ui){},rotate:function(event,ui){},stop:function(event,ui){}}),oldwidth=0;_.each(vm.front_design,function(front){front.z_index==vm.selected_text.z_index?($("#tools"+vm.selected_text.z_index+"front").css({visibility:"visible"}),$("#box"+vm.selected_text.z_index+"front").addClass("border"),intervals.push($interval(function(){collision($("#box"+vm.selected_text.z_index+"front svg"),$(".printable-area"))?$(".printable-area").removeClass("crossing_border"):$(".printable-area").addClass("crossing_border")},200)),console.log(vm.interval_id),$("#box"+vm.selected_text.z_index+"front").resizable({handles:{nw:$("#resize"+vm.selected_text.z_index+"front")},aspectRatio:!0,minHeight:15,minWidth:15,start:function(event,ui){oldwidth=ui.size.width},resize:function(event,ui){$(".printable-area").css({overflow:"hidden"});var e=ui.element,o=e.parent().parent(),left=o.css("left");$(this).css({top:parseInt(ui.position.top,10)+(ui.originalSize.height-ui.size.height)/2,left:parseInt(ui.position.left,10)+(ui.originalSize.width-ui.size.width)/2}),left=parseInt(left.replace("px",""));var top=o.css("top");top=parseInt(top.replace("px",""));var width=o.css("width");width=parseInt(width.replace("px",""));var height=o.css("height");height=parseInt(height.replace("px",""));var $left=parseInt(ui.position.left),$top=parseInt(ui.position.top),$width=parseInt(ui.size.width),$height=parseInt(ui.size.height);($left+$width>width||$top+$height>height)&&(parseInt(left+$left+$width)>490||parseInt(top+$top+$height)>490);var svg=e.find("svg");svg[0].setAttributeNS(null,"width",$width),svg[0].setAttributeNS(null,"height",$height),svg[0].setAttributeNS(null,"preserveAspectRatio","none");var svg=e.find("svg");jQuery("#"+e.data("type")+"-width").val(parseInt($width)),jQuery("#"+e.data("type")+"-height").val(parseInt($height))},stop:function(e,ui){$(".printable-area").css({overflow:"visible"})}}).rotatable(params,{handle:$("#box"+vm.selected_text.z_index+"front .icon-rotate-free")}),$("#box"+vm.selected_text.z_index+"front .rotate a").append($("#box"+vm.selected_text.z_index+"front .icon-rotate-free")),$("#box"+vm.selected_text.z_index+"front").draggable({cancel:".delete,.drag",drag:function(e,ui){$(".printable-area").css({overflow:"hidden"}),y2=ui.position.top,x2=ui.position.left;var printableWidth=$(".printable-area").width()+20;x2>printableWidth},stop:function(e,ui){$(".printable-area").css({overflow:"visible"})}}),front.selected=!0):($("#tools"+front.z_index+"front").css({visibility:"hidden"}),$("#box"+front.z_index+"front").removeClass("border"),front.selected=!1)}),$scope.focus=!0}else if(2===vm.default_tab){_index=_.findIndex(vm.back_design,function(x){return x.z_index==index}),vm.selected_text=vm.back_design[_index],vm.default_design_tab="svg"===vm.selected_text.type?2:3;var params=(document.querySelector("#box"+vm.selected_text.z_index+"back tspan"),{start:function(event,ui){},rotate:function(event,ui){},stop:function(event,ui){}}),oldwidth=0;_.each(vm.back_design,function(front){front.z_index==vm.selected_text.z_index?($("#tools"+vm.selected_text.z_index+"back").css({visibility:"visible"}),$("#box"+vm.selected_text.z_index+"back").addClass("border"),intervals.push($interval(function(){collision($("#box"+vm.selected_text.z_index+"back svg"),$(".printable-area"))?$(".printable-area").removeClass("crossing_border"):$(".printable-area").addClass("crossing_border")},200)),$("#box"+vm.selected_text.z_index+"back").resizable({handles:{nw:$("#resize"+vm.selected_text.z_index+"back")},aspectRatio:!0,minHeight:15,minWidth:15,start:function(event,ui){oldwidth=ui.size.width},resize:function(event,ui){$(".printable-area").css({overflow:"hidden"});var e=ui.element,o=e.parent().parent(),left=o.css("left");left=parseInt(left.replace("px",""));var top=o.css("top");top=parseInt(top.replace("px",""));var width=o.css("width");width=parseInt(width.replace("px","")),$(this).css({top:parseInt(ui.position.top,10)+(ui.originalSize.height-ui.size.height)/2,left:parseInt(ui.position.left,10)+(ui.originalSize.width-ui.size.width)/2});var height=o.css("height");height=parseInt(height.replace("px",""));var $left=parseInt(ui.position.left),$top=parseInt(ui.position.top),$width=parseInt(ui.size.width),$height=parseInt(ui.size.height);($left+$width>width||$top+$height>height)&&(parseInt(left+$left+$width)>490||parseInt(top+$top+$height)>490),console.log(e);var svg=e.find("svg");console.log(svg),svg[0].setAttributeNS(null,"width",$width),svg[0].setAttributeNS(null,"height",$height),svg[0].setAttributeNS(null,"preserveAspectRatio","none"),jQuery("#"+e.data("type")+"-width").val(parseInt($width)),jQuery("#"+e.data("type")+"-height").val(parseInt($height))},stop:function(e,ui){$(".printable-area").css({overflow:"visible"})}}).rotatable(params,{handle:$("#box"+vm.selected_text.z_index+"back .icon-rotate-free")}),$("#box"+vm.selected_text.z_index+"back .rotate a").append($("#box"+vm.selected_text.z_index+"back .icon-rotate-free")),$("#box"+vm.selected_text.z_index+"back").draggable({cancel:".delete,.drag",drag:function(e,ui){$(".printable-area").css({overflow:"hidden"}),y2=ui.position.top,x2=ui.position.left;var printableWidth=$(".printable-area").width()+20;x2>printableWidth&&console.log(printableWidth,x2)},stop:function(e,ui){$(".printable-area").css({overflow:"visible"})}}),front.selected=!0):($("#tools"+front.z_index+"back").css({visibility:"hidden"}),$("#box"+front.z_index+"back").removeClass("border"),front.selected=!1)}),$scope.focus=!0}else if(3===vm.default_tab){_index=_.findIndex(vm.left_design,function(x){return x.z_index==index}),vm.selected_text=vm.left_design[_index],vm.default_design_tab="svg"===vm.selected_text.type?2:3;var params=(document.querySelector("#box"+vm.selected_text.z_index+"left tspan"),{start:function(event,ui){},rotate:function(event,ui){},stop:function(event,ui){}}),oldwidth=0;_.each(vm.left_design,function(front){front.z_index==vm.selected_text.z_index?($("#tools"+vm.selected_text.z_index+"left").css({visibility:"visible"}),$("#box"+vm.selected_text.z_index+"left").addClass("border"),intervals.push($interval(function(){collision($("#box"+vm.selected_text.z_index+"left svg"),$(".printable-area"))?$(".printable-area").removeClass("crossing_border"):$(".printable-area").addClass("crossing_border")},200)),$("#box"+vm.selected_text.z_index+"left").resizable({handles:{nw:$("#resize"+vm.selected_text.z_index+"left")},aspectRatio:!0,minHeight:15,minWidth:15,start:function(event,ui){oldwidth=ui.size.width},resize:function(event,ui){$(".printable-area").css({overflow:"hidden"});var e=ui.element,o=e.parent().parent(),left=o.css("left");left=parseInt(left.replace("px",""));var top=o.css("top");top=parseInt(top.replace("px",""));var width=o.css("width");width=parseInt(width.replace("px",""));var height=o.css("height");height=parseInt(height.replace("px","")),$(this).css({top:parseInt(ui.position.top,10)+(ui.originalSize.height-ui.size.height)/2,left:parseInt(ui.position.left,10)+(ui.originalSize.width-ui.size.width)/2});var $left=parseInt(ui.position.left),$top=parseInt(ui.position.top),$width=parseInt(ui.size.width),$height=parseInt(ui.size.height);($left+$width>width||$top+$height>height)&&(parseInt(left+$left+$width)>490||parseInt(top+$top+$height)>490);var svg=e.find("svg");svg[0].setAttributeNS(null,"width",$width),svg[0].setAttributeNS(null,"height",$height),svg[0].setAttributeNS(null,"preserveAspectRatio","none"),jQuery("#"+e.data("type")+"-width").val(parseInt($width)),jQuery("#"+e.data("type")+"-height").val(parseInt($height))},stop:function(e,ui){$(".printable-area").css({overflow:"visible"})}}).rotatable(params,{handle:$("#box"+vm.selected_text.z_index+"left .icon-rotate-free")}),$("#box"+vm.selected_text.z_index+"left .rotate a").append($("#box"+vm.selected_text.z_index+"left .icon-rotate-free")),$("#box"+vm.selected_text.z_index+"left").draggable({cancel:".delete,.drag",drag:function(e,ui){$(".printable-area").css({overflow:"hidden"}),y2=ui.position.top,x2=ui.position.left;var printableWidth=$(".printable-area").width()+20;x2>printableWidth&&console.log(printableWidth,x2)},stop:function(e,ui){$(".printable-area").css({overflow:"visible"})}}),front.selected=!0):($("#tools"+front.z_index+"left").css({visibility:"hidden"}),$("#box"+front.z_index+"left").removeClass("border"),front.selected=!1)}),$scope.focus=!0}else if(4===vm.default_tab){_index=_.findIndex(vm.right_design,function(x){return x.z_index==index}),vm.selected_text=vm.right_design[_index],vm.default_design_tab="svg"===vm.selected_text.type?2:3;var params=(document.querySelector("#box"+vm.selected_text.z_index+"right tspan"),{start:function(event,ui){},rotate:function(event,ui){},stop:function(event,ui){}}),oldwidth=0;_.each(vm.right_design,function(front){front.z_index==vm.selected_text.z_index?($("#tools"+vm.selected_text.z_index+"right").css({visibility:"visible"}),$("#box"+vm.selected_text.z_index+"right").addClass("border"),intervals.push($interval(function(){collision($("#box"+vm.selected_text.z_index+"right svg"),$(".printable-area"))?$(".printable-area").removeClass("crossing_border"):$(".printable-area").addClass("crossing_border")},200)),$("#box"+vm.selected_text.z_index+"right").resizable({handles:{nw:$("#resize"+vm.selected_text.z_index+"right")},aspectRatio:!0,minHeight:15,minWidth:15,start:function(event,ui){oldwidth=ui.size.width},resize:function(event,ui){$(".printable-area").css({overflow:"hidden"});var e=ui.element,o=e.parent().parent(),left=o.css("left");left=parseInt(left.replace("px",""));var top=o.css("top");top=parseInt(top.replace("px",""));var width=o.css("width");width=parseInt(width.replace("px",""));var height=o.css("height");height=parseInt(height.replace("px","")),$(this).css({top:parseInt(ui.position.top,10)+(ui.originalSize.height-ui.size.height)/2,left:parseInt(ui.position.left,10)+(ui.originalSize.width-ui.size.width)/2});var $left=parseInt(ui.position.left),$top=parseInt(ui.position.top),$width=parseInt(ui.size.width),$height=parseInt(ui.size.height);($left+$width>width||$top+$height>height)&&(parseInt(left+$left+$width)>490||parseInt(top+$top+$height)>490);var svg=e.find("svg");svg[0].setAttributeNS(null,"width",$width),svg[0].setAttributeNS(null,"height",$height),svg[0].setAttributeNS(null,"preserveAspectRatio","none"),jQuery("#"+e.data("type")+"-width").val(parseInt($width)),jQuery("#"+e.data("type")+"-height").val(parseInt($height))},stop:function(e,ui){$(".printable-area").css({overflow:"visible"})}}).rotatable(params,{handle:$("#box"+vm.selected_text.z_index+"right .icon-rotate-free")}),$("#box"+vm.selected_text.z_index+"right .rotate a").append($("#box"+vm.selected_text.z_index+"right .icon-rotate-free")),$("#box"+vm.selected_text.z_index+"right").draggable({cancel:".delete,.drag",drag:function(e,ui){$(".printable-area").css({overflow:"hidden"}),y2=ui.position.top,x2=ui.position.left;var printableWidth=$(".printable-area").width()+20;x2>printableWidth&&console.log(printableWidth,x2)},stop:function(e,ui){$(".printable-area").css({overflow:"visible"})}}),front.selected=!0):($("#tools"+front.z_index+"right").css({visibility:"hidden"}),$("#box"+front.z_index+"right").removeClass("border"),front.selected=!1)}),$scope.focus=!0}},vm.deselect_text=function(){vm.selected_text={},$scope.focus=!1,angular.forEach(intervals,function(interval){$interval.cancel(interval)}),intervals.length=0},vm.updateText=function(){var unitWidth=0,unitHeight=0;if(1==vm.default_tab){if(vm.selected_text.z_index&&"text"==vm.selected_text.type){var text=document.querySelectorAll("#box"+vm.selected_text.z_index+"front tspan"),$w=parseInt(text[0].getBoundingClientRect().width)+13,$h=parseInt(text[0].getBoundingClientRect().height);$("#box"+vm.selected_text.z_index+"front").css("width",$w+"px"),$("#box"+vm.selected_text.z_index+"front").css("height",$h+"px");var svg=document.querySelectorAll("#box"+vm.selected_text.z_index+"front svg"),width=svg[0].getAttribute("width"),height=svg[0].getAttribute("height"),view=svg[0].getAttribute("viewBox").split(" "),vw=view[2]*$w/width,vh=view[3]*$h/height;svg[0].setAttributeNS(null,"width",$w),svg[0].setAttributeNS(null,"height",$h),svg[0].setAttributeNS(null,"viewBox","0 0 "+vw+" "+vh)}if(!vm.selected_text.z_index){var z_index=0,obj={type:"text",font:"arial",color:"rgb(0,0,0)",text:vm.selected_text.text,z_index:z_index,selected:!1,slider:{value:0,options:{floor:-360,ceil:360,step:1}}};vm.front_design.length?obj.z_index=vm.front_design[vm.front_design.length-1].z_index+1:obj.z_index=1,vm.front_design.push(obj),_.each(vm.front_design,function(front){front.z_index!=obj.z_index?($("#tools"+front.z_index+"front").css({visibility:"hidden"}),$("#box"+front.z_index+"front").removeClass("border"),front.selected=!1):setTimeout(function(){var text=document.querySelectorAll("#box"+obj.z_index+"front tspan"),$w=parseInt(text[0].getBoundingClientRect().width),$h=parseInt(text[0].getBoundingClientRect().height);unitWidth=$w,unitHeight=$h,$("#box"+obj.z_index+"front").css("width",$w+$w+"px"),$("#box"+obj.z_index+"front").css("height",$h+"px");var svg=document.querySelectorAll("#box"+obj.z_index+"front svg");svg[0].getAttribute("width"),svg[0].getAttribute("height");svg[0].setAttributeNS(null,"width",$w),svg[0].setAttributeNS(null,"height",$h),svg[0].setAttributeNS(null,"viewBox","0 0 "+$w+" "+$h),$("#tools"+obj.z_index+"front").css({visibility:"visible"}),$("#box"+obj.z_index+"front").addClass("border"),front.selected=!0,vm.select_text(obj.z_index)},0)}),$scope.focus=!0,vm.selected_text=obj}}else if(2==vm.default_tab){if(vm.selected_text.z_index&&"text"==vm.selected_text.type){var text=document.querySelectorAll("#box"+vm.selected_text.z_index+"back tspan"),$w=parseInt(text[0].getBoundingClientRect().width)+13,$h=parseInt(text[0].getBoundingClientRect().height);$("#box"+vm.selected_text.z_index+"back").css("width",$w+"px"),$("#box"+vm.selected_text.z_index+"back").css("height",$h+"px");var svg=document.querySelectorAll("#box"+vm.selected_text.z_index+"back svg"),width=svg[0].getAttribute("width"),height=svg[0].getAttribute("height"),view=svg[0].getAttribute("viewBox").split(" "),vw=view[2]*$w/width,vh=view[3]*$h/height;svg[0].setAttributeNS(null,"width",$w),svg[0].setAttributeNS(null,"height",$h),svg[0].setAttributeNS(null,"viewBox","0 0 "+vw+" "+vh)}if(!vm.selected_text.z_index){var z_index=0,obj={type:"text",font:"arial",color:"rgb(0,0,0)",text:vm.selected_text.text,z_index:z_index,selected:!1,slider:{value:0,options:{floor:-360,ceil:360,step:1}}};vm.back_design.length?obj.z_index=vm.back_design[vm.back_design.length-1].z_index+1:obj.z_index=1,vm.back_design.push(obj),_.each(vm.back_design,function(front){front.z_index!=obj.z_index?($("#tools"+front.z_index+"back").css({visibility:"hidden"}),$("#box"+front.z_index+"back").removeClass("border"),front.selected=!1):setTimeout(function(){var text=document.querySelectorAll("#box"+obj.z_index+"back tspan"),$w=parseInt(text[0].getBoundingClientRect().width),$h=parseInt(text[0].getBoundingClientRect().height);unitWidth=$w,unitHeight=$h,$("#box"+obj.z_index+"back").css("width",$w+$w+"px"),$("#box"+obj.z_index+"back").css("height",$h+"px");var svg=document.querySelectorAll("#box"+obj.z_index+"back svg");svg[0].getAttribute("width"),svg[0].getAttribute("height");svg[0].setAttributeNS(null,"width",$w),svg[0].setAttributeNS(null,"height",$h),svg[0].setAttributeNS(null,"viewBox","0 0 "+$w+" "+$h),$("#tools"+obj.z_index+"back").css({visibility:"visible"}),$("#box"+obj.z_index+"back").addClass("border"),front.selected=!0,vm.select_text(obj.z_index)},0)}),$scope.focus=!0,vm.selected_text=obj}}else if(3==vm.default_tab){if(vm.selected_text.z_index&&"text"==vm.selected_text.type){var text=document.querySelectorAll("#box"+vm.selected_text.z_index+"left tspan"),$w=parseInt(text[0].getBoundingClientRect().width)+13,$h=parseInt(text[0].getBoundingClientRect().height);$("#box"+vm.selected_text.z_index+"left").css("width",$w+"px"),$("#box"+vm.selected_text.z_index+"left").css("height",$h+"px");var svg=document.querySelectorAll("#box"+vm.selected_text.z_index+"left svg"),width=svg[0].getAttribute("width"),height=svg[0].getAttribute("height"),view=svg[0].getAttribute("viewBox").split(" "),vw=view[2]*$w/width,vh=view[3]*$h/height;svg[0].setAttributeNS(null,"width",$w),svg[0].setAttributeNS(null,"height",$h),svg[0].setAttributeNS(null,"viewBox","0 0 "+vw+" "+vh)}if(!vm.selected_text.z_index){var z_index=0,obj={type:"text",font:"arial",color:"rgb(0,0,0)",text:vm.selected_text.text,z_index:z_index,selected:!1,slider:{value:0,options:{floor:-360,ceil:360,step:1}}};vm.left_design.length?obj.z_index=vm.left_design[vm.left_design.length-1].z_index+1:obj.z_index=1,vm.left_design.push(obj),_.each(vm.left_design,function(front){front.z_index!=obj.z_index?($("#tools"+front.z_index+"left").css({visibility:"hidden"}),$("#box"+front.z_index+"left").removeClass("border"),front.selected=!1):setTimeout(function(){var text=document.querySelectorAll("#box"+obj.z_index+"left tspan"),$w=parseInt(text[0].getBoundingClientRect().width),$h=parseInt(text[0].getBoundingClientRect().height);unitWidth=$w,unitHeight=$h,$("#box"+obj.z_index+"left").css("width",$w+$w+"px"),$("#box"+obj.z_index+"left").css("height",$h+"px");var svg=document.querySelectorAll("#box"+obj.z_index+"left svg");svg[0].getAttribute("width"),svg[0].getAttribute("height");svg[0].setAttributeNS(null,"width",$w),svg[0].setAttributeNS(null,"height",$h),svg[0].setAttributeNS(null,"viewBox","0 0 "+$w+" "+$h),$("#tools"+obj.z_index+"left").css({visibility:"visible"}),$("#box"+obj.z_index+"left").addClass("border"),front.selected=!0,vm.select_text(obj.z_index)},0)}),$scope.focus=!0,vm.selected_text=obj}}else if(4==vm.default_tab){if(vm.selected_text.z_index&&"text"==vm.selected_text.type){var text=document.querySelectorAll("#box"+vm.selected_text.z_index+"right tspan"),$w=parseInt(text[0].getBoundingClientRect().width)+13,$h=parseInt(text[0].getBoundingClientRect().height);$("#box"+vm.selected_text.z_index+"right").css("width",$w+"px"),$("#box"+vm.selected_text.z_index+"right").css("height",$h+"px");var svg=document.querySelectorAll("#box"+vm.selected_text.z_index+"right svg"),width=svg[0].getAttribute("width"),height=svg[0].getAttribute("height"),view=svg[0].getAttribute("viewBox").split(" "),vw=view[2]*$w/width,vh=view[3]*$h/height;svg[0].setAttributeNS(null,"width",$w),svg[0].setAttributeNS(null,"height",$h),svg[0].setAttributeNS(null,"viewBox","0 0 "+vw+" "+vh)}if(!vm.selected_text.z_index){var z_index=0,obj={type:"text",font:"arial",color:"rgb(0,0,0)",text:vm.selected_text.text,z_index:z_index,selected:!1,slider:{value:0,options:{floor:-360,ceil:360,step:1}}};vm.right_design.length?obj.z_index=vm.right_design[vm.right_design.length-1].z_index+1:obj.z_index=1,vm.right_design.push(obj),_.each(vm.right_design,function(front){front.z_index!=obj.z_index?($("#tools"+front.z_index+"right").css({visibility:"hidden"}),$("#box"+front.z_index+"right").removeClass("border"),front.selected=!1):setTimeout(function(){var text=document.querySelectorAll("#box"+obj.z_index+"right tspan"),$w=parseInt(text[0].getBoundingClientRect().width),$h=parseInt(text[0].getBoundingClientRect().height);unitWidth=$w,unitHeight=$h,$("#box"+obj.z_index+"right").css("width",$w+$w+"px"),$("#box"+obj.z_index+"right").css("height",$h+"px");var svg=document.querySelectorAll("#box"+obj.z_index+"right svg");svg[0].getAttribute("width"),svg[0].getAttribute("height");svg[0].setAttributeNS(null,"width",$w),svg[0].setAttributeNS(null,"height",$h),svg[0].setAttributeNS(null,"viewBox","0 0 "+$w+" "+$h),$("#tools"+obj.z_index+"right").css({visibility:"visible"}),$("#box"+obj.z_index+"right").addClass("border"),front.selected=!0,vm.select_text(obj.z_index)},0)}),$scope.focus=!0,vm.selected_text=obj}}},vm.blockEnter=function(event){if(13===event.keyCode)return event.preventDefault(),!1},$scope.slider={value:0,options:{floor:-360,ceil:360,step:1}},vm.onDrag=function(value){vm.currentRotation=value},vm.onDragEnd=function(value){vm.currentRotation=value,console.log("DRAG_END",value)},vm.mousedown=function(){vm.rotation=!0},vm.mouseup=function(){vm.rotation=!1,console.log("Fireeeddd")},vm.mousemove=function(e){console.log(vm.rotation);$("#box");vm.rotation}}function AnimationService(){function getRandomInt(min,max){return Math.floor(Math.random()*(max-min+1))+min}var animations=["bounce","flash","pulse","rubberBand","shake","headShake","swing","tada","wobble","jello","bounceIn","bounceInDown","bounceInLeft","bounceInRight","bounceInUp","bounceOut","bounceOutDown","bounceOutLeft","bounceOutRight","bounceOutUp","fadeIn","fadeInDown","fadeInDownBig","fadeInLeft","fadeInLeftBig","fadeInRight","fadeInRightBig","fadeInUp","fadeInUpBig","fadeOut","fadeOutDown","fadeOutDownBig","fadeOutLeft","fadeOutLeftBig","fadeOutRight","fadeOutRightBig","fadeOutUp","fadeOutUpBig","flipInX","flipInY","flipOutX","flipOutY","lightSpeedIn","lightSpeedOut","rotateIn","rotateInDownLeft","rotateInDownRight","rotateInUpLeft","rotateInUpRight","rotateOut","rotateOutDownLeft","rotateOutDownRight","rotateOutUpLeft","rotateOutUpRight","hinge","rollIn","rollOut","zoomIn","zoomInDown","zoomInLeft","zoomInRight","zoomInUp","zoomOut","zoomOutDown","zoomOutLeft","zoomOutRight","zoomOutUp","slideInDown","slideInLeft","slideInRight","slideInUp","slideOutDown","slideOutLeft","slideOutRight","slideOutUp"],service={GetRandomInAnimation:function(){var animation="",valid=!1;do{var num=getRandomInt(1,animations.length);if(animation=animations[num],!animation)return;animation.indexOf("Out")==-1&&(valid=!0)}while(!valid);return animation},GetRandomOutAnimation:function(){var animation="",valid=!1;do{var num=getRandomInt(1,animations.length);animation=animations[num],animation.indexOf("In")==-1&&(valid=!0)}while(!valid);return animation},GetRandomAnimation:function(){var animation="",num=getRandomInt(1,animations.length);return animation=animations[num],valid=!0,animation}};return service}function HttpService($q,$http){var _Get=function(url,reqData){return _SendRequest("GET",url,reqData)},_Post=function(url,reqData){return _SendRequest("POST",url,reqData)},_Put=function(url,reqData){return _SendRequest("PUT",url,reqData)},_Delete=function(url,reqData){return _SendRequest("Delete",url,reqData)},_SendRequest=function(method,url,reqData){var req="";req={method:method,url:baseURL+url,data:reqData,headers:{"Content-Type":"application/x-www-form-urlencoded"},transformRequest:function(obj){var str=[];for(var p in obj)str.push(encodeURIComponent(p)+"="+encodeURIComponent(obj[p]));return str.join("&")}};var deferred=$q.defer(),_successCallback=function(data){deferred.resolve(data)},_errorCallback=function(data){};return console.log("API URl "+req.url),console.log("Req "+JSON.stringify(req.data)),$http(req).success(_successCallback).error(_errorCallback),deferred.promise},service={Get:_Get,Post:_Post,Put:_Put,Delete:_Delete};return service}function DashboardService($q,$http,HttpService,$timeout){var key="fstky2e4mdt",_GetGraphics=function(){var deferred=$q.defer();return HttpService.Post("/art_random",{key:key}).then(function(data){return deferred.resolve(data),deferred.promise},function(err){return deferred.reject(err),deferred.promise}),deferred.promise},_GetShirtStyle=function(){var deferred=$q.defer();return HttpService.Post("/styles",{key:key}).then(function(data){return console.log(data),deferred.resolve(data),deferred.promise},function(err){return deferred.reject(err),deferred.promise}),deferred.promise},_GetFonts=function(){var deferred=$q.defer();return HttpService.Post("/fonts",{key:key}).then(function(data){return deferred.resolve(data),deferred.promise},function(err){return deferred.reject(err),deferred.promise}),deferred.promise},_GetConfig=function(){var deferred=$q.defer();return HttpService.Post("/config",{key:key}).then(function(data){return deferred.resolve(data),deferred.promise},function(err){return deferred.reject(err),deferred.promise}),deferred.promise},_GetSvg=function(url){var deferred=$q.defer();return $http.get(url).then(function(data){return deferred.resolve(data),deferred.promise},function(err){return deferred.reject(err),deferred.promise}),deferred.promise},_GetColors=function(){var deferred=$q.defer();return HttpService.Post("/colors",{key:key}).then(function(data){return deferred.resolve(data),deferred.promise},function(err){return deferred.reject(err),deferred.promise}),deferred.promise},_GetGoogleFonts=function(){var fonts=[{family:"Aclonica",category:"sans-serif",variants:["regular"],version:"v6",lastModified:"2015-04-06"},{family:"Adamina",category:"serif",variants:["regular"],version:"v7",lastModified:"2015-04-06"},{family:"Advent Pro",category:"sans-serif",variants:["100","200","300","regular","500","600","700"],version:"v4",lastModified:"2015-04-06"},{family:"Aguafina Script",category:"handwriting",variants:["regular"],version:"v5",lastModified:"2015-04-06"},{family:"Akronim",category:"display",variants:["regular"],version:"v5",lastModified:"2015-04-06"},{family:"Aladin",category:"handwriting",variants:["regular"],version:"v5",lastModified:"2015-04-06"},{family:"Alegreya Sans",category:"sans-serif",variants:["100","100italic","300","300italic","regular","italic","500","500italic","700","700italic","800","800italic","900","900italic"],version:"v3",lastModified:"2015-04-06"},{family:"Alegreya Sans SC",category:"sans-serif",variants:["100","100italic","300","300italic","regular","italic","500","500italic","700","700italic","800","800italic","900","900italic"],version:"v3",lastModified:"2015-04-06"},{family:"Alex Brush",category:"handwriting",variants:["regular"],version:"v6",lastModified:"2015-04-06"},{family:"Alfa Slab One",category:"display",variants:["regular"],version:"v5",lastModified:"2015-04-06"},{family:"Alice",category:"serif",variants:["regular"],version:"v7",lastModified:"2015-04-06"},{family:"Alike",category:"serif",variants:["regular"],version:"v7",lastModified:"2015-04-06"},{family:"Alike Angular",category:"serif",variants:["regular"],version:"v6",lastModified:"2015-04-06"},{family:"Allan",category:"display",variants:["regular","700"],version:"v7",lastModified:"2015-04-06"},{family:"Allerta",category:"sans-serif",variants:["regular"],version:"v7",lastModified:"2015-04-06"},{family:"Allerta Stencil",category:"sans-serif",variants:["regular"],version:"v7",lastModified:"2015-04-06"},{family:"Allura",category:"handwriting",variants:["regular"],version:"v4",lastModified:"2015-04-06"}];return fonts},_GetCategories=function(){var deferred=$q.defer();return HttpService.Post("/categories",{key:key}).then(function(data){return deferred.resolve(data),deferred.promise},function(err){deferred.reject(err)}),deferred.promise},_SearchArts=function(query,page){var deferred=$q.defer();return HttpService.Post("/art_search",{key:key,page:page,query:query}).then(function(data){return deferred.resolve(data),deferred.promise},function(err){deferred.reject(err)}),deferred.promise},_LaunchCompaign=function(obj){var deferred=$q.defer();return obj.key=key,HttpService.Post("/save",obj,{key:key}).then(function(data){return deferred.resolve(data),deferred.promise},function(err){deferred.reject(err)}),deferred.promise},_signUp=function(obj){var deferred=$q.defer();return obj.key=key,HttpService.Post("/signup",obj,{key:key}).then(function(data){return deferred.resolve(data),deferred.promise},function(err){deferred.reject(err)}),deferred.promise},_login=function(obj){var deferred=$q.defer();return obj.key=key,HttpService.Post("/login",obj).then(function(data){return deferred.resolve(data),deferred.promise},function(err){deferred.reject(err)}),deferred.promise},_validateUrl=function(url){var deferred=$q.defer();return HttpService.Post("/available",{key:key,url:url}).then(function(data){return deferred.resolve(data),deferred.promise},function(err){deferred.reject(err)}),deferred.promise},services={GetGraphics:_GetGraphics,GetShirtStyle:_GetShirtStyle,GetFonts:_GetFonts,GetGoogleFonts:_GetGoogleFonts,GetColors:_GetColors,GetConfig:_GetConfig,GetSvg:_GetSvg,GetCategories:_GetCategories,SearchArts:_SearchArts,LaunchCompaign:_LaunchCompaign,SignUp:_signUp,Login:_login,ValidateURL:_validateUrl};return services}function twClickOutside($window,$parse){return{link:function(scope,el,attr){if(attr.twClickOutside){var ignore;attr.ignoreIf&&(ignore=$parse(attr.ignoreIf));var nakedEl=el[0],fn=$parse(attr.twClickOutside),handler=function(e){nakedEl===e.target||nakedEl.contains(e.target)||ignore&&ignore(scope)||scope.$apply(fn)};$window.addEventListener("click",handler,!0),scope.$on("$destroy",function(e){$window.removeEventListener("click",handler)})}}}}function NavController($scope,$rootScope){}if(!function(a,b){"object"==typeof module&&"object"==typeof module.exports?module.exports=a.document?b(a,!0):function(a){if(!a.document)throw new Error("jQuery requires a window with a document");return b(a)}:b(a)}("undefined"!=typeof window?window:this,function(a,b){function r(a){var b="length"in a&&a.length,c=m.type(a);return"function"!==c&&!m.isWindow(a)&&(!(1!==a.nodeType||!b)||("array"===c||0===b||"number"==typeof b&&b>0&&b-1 in a))}function w(a,b,c){if(m.isFunction(b))return m.grep(a,function(a,d){return!!b.call(a,d,a)!==c});if(b.nodeType)return m.grep(a,function(a){return a===b!==c});if("string"==typeof b){if(v.test(b))return m.filter(b,a,c);b=m.filter(b,a)}return m.grep(a,function(a){return m.inArray(a,b)>=0!==c})}function D(a,b){do a=a[b];while(a&&1!==a.nodeType);return a}function G(a){var b=F[a]={};return m.each(a.match(E)||[],function(a,c){b[c]=!0}),b}function I(){y.addEventListener?(y.removeEventListener("DOMContentLoaded",J,!1),a.removeEventListener("load",J,!1)):(y.detachEvent("onreadystatechange",J),a.detachEvent("onload",J))}function J(){(y.addEventListener||"load"===event.type||"complete"===y.readyState)&&(I(),m.ready())}function O(a,b,c){if(void 0===c&&1===a.nodeType){var d="data-"+b.replace(N,"-$1").toLowerCase();if(c=a.getAttribute(d),"string"==typeof c){try{c="true"===c||"false"!==c&&("null"===c?null:+c+""===c?+c:M.test(c)?m.parseJSON(c):c)}catch(e){}m.data(a,b,c)}else c=void 0}return c}function P(a){var b;for(b in a)if(("data"!==b||!m.isEmptyObject(a[b]))&&"toJSON"!==b)return!1;return!0}function Q(a,b,d,e){if(m.acceptData(a)){var f,g,h=m.expando,i=a.nodeType,j=i?m.cache:a,k=i?a[h]:a[h]&&h;if(k&&j[k]&&(e||j[k].data)||void 0!==d||"string"!=typeof b)return k||(k=i?a[h]=c.pop()||m.guid++:h),j[k]||(j[k]=i?{}:{toJSON:m.noop}),("object"==typeof b||"function"==typeof b)&&(e?j[k]=m.extend(j[k],b):j[k].data=m.extend(j[k].data,b)),g=j[k],e||(g.data||(g.data={}),g=g.data),void 0!==d&&(g[m.camelCase(b)]=d),"string"==typeof b?(f=g[b],null==f&&(f=g[m.camelCase(b)])):f=g,f}}function R(a,b,c){if(m.acceptData(a)){
